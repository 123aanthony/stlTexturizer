...
import { readPlacement, readPoint, readDirection } from "./placement.js";
import { Table, num, ref, list, numList } from "../step/entities.js";
...
class Plane {
    ...
    normal() { return this.f.z; }
    curvatureRadius() { return Infinity; }
}
class Cylinder {
    ...
    f;
    r;
    constructor(f, r) { this.f = f; this.r = r; }
    evaluate(u, v) {
    ...
    curvatureRadius() { return this.r; } // tight in u, flat in v
}
class Cone {
...
class Torus {
    ...
    rMaj;
    rMin;
    constructor(f, rMaj, rMin) { this.f = f; this.rMaj = rMaj; this.rMin = rMin; }
    evaluate(u, v) {
        const ring = add(scale(this.f.x, Math.cos(u)), scale(this.f.y, Math.sin(u)));
    ...
    curvatureRadius() { return this.rMin; } // tube radius dominates
}
/**
...
class DegenerateTorus {
    ...
    project(p) {
        ...
        if (this.outer)
            return [Math.atan2(ly, lx), Math.atan2(lz, rho - this.rMaj)];
...
class OffsetSurface {
    ...
    uDomain;
    vDomain;
    constructor(base, dist) {
        this.base = base;
        this.dist = dist;
        this.periodicU = base.periodicU;
        this.periodicV = base.periodicV;
        this.uPeriod = base.uPeriod;
        this.vPeriod = base.vPeriod;
        this.uSeam = base.uSeam;
        this.vSeam = base.vSeam;
        this.uDomain = base.uDomain;
        this.vDomain = base.vDomain;
    }
    evaluate(u, v) {
        const p = this.base.evaluate(u, v), n = this.base.normal(u, v);
        return [p[0] + n[0] * this.dist, p[1] + n[1] * this.dist, p[2] + n[2] * this.dist];
    }
    project(p, hu, hv) { return this.base.project(p, hu, hv); }
    normal(u, v) { return this.base.normal(u, v); }
    curvatureRadius(u, v) {
...
class ExtrusionSurface {
    ...
    curve;
    axis; // full extrusion vector (direction · magnitude, mm)
    axisUnit;
    axisLen;
    periodicU;
    uPeriod;
    uSeam;
    constructor(curve, axis) {
        this.curve = curve;
        this.axis = axis;
        ...
        if (curve.closed) {
            this.uPeriod = curve.t1 - curve.t0;
            this.uSeam = curve.t0;
        }
    }
    evaluate(u, v) {
    ...
    project(p) {
        ...
        for (let it = 0; it < 3; it++) {
            ...
            const q = [p[0] - this.axis[0] * v, p[1] - this.axis[1] * v, p[2] - this.axis[2] * v];
            const u2 = this.curve.project(q);
            if (Math.abs(u2 - u) < 1e-10) {
                u = u2;
                break;
            }
            u = u2;
    ...
    curvatureRadius() { return this.curve.minRadius(); }
}
...
class RevolutionSurface {
    ...
    evaluate(u, v) {
        if (this.periodicV && this.vPeriod)
            v = this.t0 + (((v - this.t0) % this.vPeriod) + this.vPeriod) % this.vPeriod;
    ...
    project(p) {
        ...
        const cost = (v) => { const [a, rho] = this.axialRadial(this.curve.evaluate(v)); const da = a - ap, dr = rho - rp; return da * da + dr * dr; };
        ...
        if (nP < 1e-9 || nC < 1e-9)
            return [0, v]; // on the axis (pole): angle is arbitrary
...
function findSpan(degree, nCtrl, knots, u) {
    const nmax = nCtrl - 1;
    let k = degree;
    while (k < nmax && knots[k + 1] <= u)
        k++;
    return k;
}
...
function deBoorTri(degree, knots, u, k, dst) {
...
function expandKnots(mults, vals) {
    const k = [];
    for (let i = 0; i < vals.length; i++)
        for (let j = 0; j < (mults[i] ?? 0); j++)
            k.push(vals[i]);
    return k;
}
class BSplineSurface {
    ...
    gU = [];
    gV = [];
    gPF = new Float64Array(0);
    rc = Infinity;
    rcGrid = []; // per grid CELL local min normal-turn radius (same estimate as rc)
    closedU = false;
    closedV = false; // patch wraps onto itself in u / v
    constructor(uDeg, vDeg, cps, uKnots, vKnots) {
        this.uDeg = uDeg;
        this.vDeg = vDeg;
        this.uKnots = uKnots;
        this.vKnots = vKnots;
        ...
        this.v0 = vKnots[vDeg];
        this.v1 = vKnots[nv + 1];
        this.uDomain = [this.u0, this.u1];
        this.vDomain = [this.v0, this.v1];
        ...
        this.periodicU = this.closedU;
        this.periodicV = this.closedV;
        this.uPeriod = this.u1 - this.u0;
        this.vPeriod = this.v1 - this.v0;
        this.uSeam = this.u0;
        this.vSeam = this.v0; // knot-domain start = where a closed patch wraps
        this.buildGrid();
    }
    buildGrid() {
        ...
        for (let i = 0; i <= GU; i++)
            this.gU.push(this.u0 + ((this.u1 - this.u0) * i) / GU);
        for (let j = 0; j <= GV; j++)
            this.gV.push(this.v0 + ((this.v1 - this.v0) * j) / GV);
        ...
        const RU = 24, RV = 24;
        const rU = (i) => this.u0 + ((this.u1 - this.u0) * i) / RU;
        const rV = (j) => this.v0 + ((this.v1 - this.v0) * j) / RV;
        let minR = Infinity;
        for (let i = 0; i < RU; i++) {
            const row = [];
            for (let j = 0; j < RV; j++) {
    ...
    evaluateRaw(u, v) {
        ...
        if (dbRow.length < (uDeg + 1) * 4)
            dbRow = new Float64Array((uDeg + 1) * 4);
        if (dbTri.length < (vDeg + 1) * 4)
            dbTri = new Float64Array((vDeg + 1) * 4);
        ...
        for (let j = 0; j <= uDeg; j++) {
            ...
            for (let c = 0; c < nvals; c++)
                dbTri[c] = this.cpsF[rowOff + c];
            deBoorTri(vDeg, this.vKnots, v, kv, dbTri);
            const res = vDeg * 4, o = j * 4;
            dbRow[o] = dbTri[res];
            dbRow[o + 1] = dbTri[res + 1];
            dbRow[o + 2] = dbTri[res + 2];
            dbRow[o + 3] = dbTri[res + 3];
        }
    ...
    evaluate(u, v) {
        ...
        if (this.closedU && this.uPeriod > 0)
            u = this.u0 + (((u - this.u0) % this.uPeriod) + this.uPeriod) % this.uPeriod;
        if (this.closedV && this.vPeriod > 0)
            v = this.v0 + (((v - this.v0) % this.vPeriod) + this.vPeriod) % this.vPeriod;
    ...
    normalAt(u, v) {
        ...
        const du = [pu1[0] - pu2[0], pu1[1] - pu2[1], pu1[2] - pu2[2]];
        const dv = [pv1[0] - pv2[0], pv1[1] - pv2[1], pv1[2] - pv2[2]];
        const n = cross(du, dv);
        const len = Math.hypot(n[0], n[1], n[2]);
        return len > 1e-12 ? [n[0] / len, n[1] / len, n[2] / len] : [0, 0, 1];
    }
    normal(u, v) { return this.normalAt(u, v); }
    curvatureRadius(u, v) {
        ...
        for (let i = Math.max(0, ci - 1); i <= Math.min(nu - 1, ci + 1); i++) {
            for (let j = Math.max(0, cj - 1); j <= Math.min(nv - 1, cj + 1); j++)
                r = Math.min(r, this.rcGrid[i][j]);
        }
            ...
            if (r2 > 0.25 * bd2) {
                ...
                for (let i = 0; i < this.gU.length; i++) {
                    for (let j = 0; j < nv; j++) {
                        ...
                        const d = (g[o] - p[0]) ** 2 + (g[o + 1] - p[1]) ** 2 + (g[o + 2] - p[2]) ** 2;
                        if (d <= lim)
                            near.push({ d, cu: this.gU[i], cv: this.gV[j] });
                    }
                }
                near.sort((a, b) => a.d - b.d);
                const cand = [];
                for (const { cu, cv } of near) {
                    if (cand.some(([x, y]) => Math.hypot(x - cu, y - cv) < span / 16))
                        continue;
                    cand.push([cu, cv]);
                    if (cand.length >= 8)
                        break;
    ...
    newton(p, u, v) {
        ...
        const clampU = (x) => this.closedU ? x : Math.min(Math.max(x, this.u0), this.u1);
        const clampV = (x) => this.closedV ? x : Math.min(Math.max(x, this.v0), this.v1);
        for (let it = 0; it < 12; it++) {
            const S = this.evaluate(u, v);
            const r = [S[0] - p[0], S[1] - p[1], S[2] - p[2]];
            ...
            const Su = [(Su0[0] - Su1[0]) * idu, (Su0[1] - Su1[1]) * idu, (Su0[2] - Su1[2]) * idu];
            const Sv = [(Sv0[0] - Sv1[0]) * idv, (Sv0[1] - Sv1[1]) * idv, (Sv0[2] - Sv1[2]) * idv];
            ...
            u = clampU(u + du);
            v = clampV(v + dv);
            if (Math.abs(du) < eu && Math.abs(dv) < ev)
                break;
        }
        return [u, v];
    }
...
function makeBSplineSurface(t, id, s) {
    ...
    const cps = cpRefs.map((row, iu) => row.map((pid, iv) => {
...
export function makeSurface(t, id, s, aRad = 1) {
    const kind = t.typeOf(id);
    if (!kind)
        return makeBSplineSurface(t, id, s); // complex (rational B-spline) surface
    const r = t.record(id);
    switch (kind) {
        ...
        case "SURFACE_OF_REVOLUTION": {
            ...
            const D = ax.params[2] && ax.params[2].k === "ref" ? readDirection(t, ref(ax.params[2])) : [0, 0, 1];
        ...
        case "DEGENERATE_TOROIDAL_SURFACE":
            ...
            return new DegenerateTorus(readPlacement(t, ref(r.params[1]), s), num(r.params[2]) * s, num(r.params[3]) * s, r.params[4]?.k === "enum" ? r.params[4].v === "T" : true);
        case "OFFSET_SURFACE": {
...
export const isSphere = (s) => s.kind === "SPHERICAL_SURFACE";
export const isBSpline = (s) => s.kind === "B_SPLINE_SURFACE";
/**
 ...
 */
export function analyzeSurface(t, id, s, aRad = 1) {
    const kind = t.typeOf(id);
    if (!kind)
        return { kind: "" };
    const r = t.record(id);
