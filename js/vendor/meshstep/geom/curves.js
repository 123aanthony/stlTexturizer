import { add, dot, scale, sub, dist, lerp } from "./vec.js";
import { readPlacement, readPoint, readDirection } from "./placement.js";
import { Table, num, ref, refList, numList } from "../step/entities.js";
const TWO_PI = Math.PI * 2;
class LineCurve {
    kind = "LINE";
    o;
    d; // d includes the STEP vector magnitude: p(t) = o + t·d
    t0 = 0;
    t1 = 1;
    closed = false;
    constructor(o, d) { this.o = o; this.d = d; }
    evaluate(t) { return add(this.o, scale(this.d, t)); }
    project(p) { const l2 = dot(this.d, this.d); return l2 > 0 ? dot(sub(p, this.o), this.d) / l2 : 0; }
    minRadius() { return Infinity; }
}
class CircleCurve {
    kind = "CIRCLE";
    f;
    ...
    evaluate(t) { return add(this.f.o, add(scale(this.f.x, Math.cos(t) * this.r), scale(this.f.y, Math.sin(t) * this.r))); }
    project(p) { const d = sub(p, this.f.o); return Math.atan2(dot(d, this.f.y), dot(d, this.f.x)); }
    minRadius() { return this.r; }
}
class EllipseCurve {
    kind = "ELLIPSE";
    f;
    ...
    t1 = TWO_PI;
    closed = true;
    constructor(f, a, b) { this.f = f; this.a = a; this.b = b; }
    evaluate(t) { return add(this.f.o, add(scale(this.f.x, Math.cos(t) * this.a), scale(this.f.y, Math.sin(t) * this.b))); }
    project(p) { const d = sub(p, this.f.o); return Math.atan2(dot(d, this.f.y) / this.b, dot(d, this.f.x) / this.a); }
    minRadius() { return Math.min(this.a, this.b) ** 2 / Math.max(this.a, this.b, 1e-9); }
}
class BSplineCurve3 {
    kind = "B_SPLINE_CURVE";
    degree;
    ...
    closed;
    samples = [];
    constructor(degree, cps, knots, weights, t0, t1) {
        this.degree = degree;
        this.cps = cps;
        this.knots = knots;
        ...
        for (let i = 0; i <= N; i++) {
            ...
            this.samples.push({ t, p: this.evaluate(t) });
        }
    }
    evaluate(t) {
    ...
    minRadius() {
        ...
        for (let i = 1; i + 1 < this.samples.length; i++) {
            ...
            if (ang > 1e-5)
                minR = Math.min(minR, ((lu + lv) / 2) / ang);
        }
        return minR;
    }
}
...
class TrimmedCurve3 {
    ...
    t1 = 1;
    closed;
    constructor(base, ta, tb) {
        this.base = base;
        this.ta = ta;
        this.tb = tb;
        this.closed = Math.abs(Math.abs(tb - ta) - (base.t1 - base.t0)) < 1e-9 && base.closed;
    }
    evaluate(t) { return this.base.evaluate(this.ta + (this.tb - this.ta) * t); }
    project(p) {
    ...
    minRadius() { return this.base.minRadius(); }
}
/**
 ...
 */
export function makeCurve(t, id, s, aRad = 1) {
    ...
    if (kind === "TRIMMED_CURVE") {
        ...
        const trim = (p) => {
            ...
            for (const q of p.v)
                if (q.k === "ref")
                    return base.project(readPoint(t, q.v, s));
            for (const q of p.v) {
                if (q.k === "num")
                    return conic ? q.v * aRad : q.v;
                if (q.k === "typed" && q.params[0]?.k === "num")
                    return conic ? q.params[0].v * aRad : q.params[0].v;
            }
            return null;
        };
    ...
    if (bs)
        return new BSplineCurve3(bs.degree, bs.cps, bs.knots, bs.weights, bs.u0, bs.u1);
    return null;
}
/**
 ...
 */
export function analyzeEdgeCurve(t, curveId, v0, v1, sameSense, s) {
    ...
    const conicSweep = (t0, t1) => {
        if (dist(v0, v1) < 1e-5)
            return sameSense ? TWO_PI : -TWO_PI;
        let d = t1 - t0;
        if (sameSense) {
            while (d <= 0)
                d += TWO_PI;
    ...
    if (kind === "CIRCLE") {
        ...
        const ang = (p) => { const d = sub(p, f.o); return Math.atan2(dot(d, f.y), dot(d, f.x)); };
    ...
    if (kind === "ELLIPSE") {
        ...
        const at = (p) => { const d = sub(p, f.o); return Math.atan2(dot(d, f.y) / b, dot(d, f.x) / a); };
 ...
 */
export function sampleCurve(c, chordTol, maxSegLen, ta = c.t0, tb = c.t1, normalDev = Math.PI) {
    const ts = [];
    const n0 = c.closed || c.kind === "TRIMMED_CURVE" ? 8 : 2;
    for (let i = 0; i <= n0; i++)
        ts.push(ta + ((tb - ta) * i) / n0);
...
function deBoor(degree, cps, knots, u, weights) {
...
function bsplineData(t, curveId, s) {
    let degree, cpsRefs, mults, knotVals, weights;
    if (!t.isComplex(curveId) && t.typeOf(curveId) === "B_SPLINE_CURVE_WITH_KNOTS") {
        ...
        degree = num(r.params[1]);
        cpsRefs = refList(r.params[2]);
        mults = numList(r.params[6]);
        knotVals = numList(r.params[7]);
        ...
        if (!bc || !bk)
            return null;
        degree = num(bc.params[0]);
        cpsRefs = refList(bc.params[1]); // (degree, cps, form, closed, self_int)
        mults = numList(bk.params[0]);
        knotVals = numList(bk.params[1]); // (mults, knots, spec)
    ...
    for (let i = 0; i < knotVals.length; i++)
        for (let j = 0; j < mults[i]; j++)
            knots.push(knotVals[i]);
    return { degree, cps, knots, weights, u0: knots[degree], u1: knots[cps.length] };
}
function arcSegments(radius, sweepAbs, chordTol, normalDev = Math.PI) {
 ...
 */
function cutOpenPolyline(pts, v0, v1) {
    const pos = (q) => {
    ...
    const out = [p0 <= p1 ? v0 : v1];
    for (let i = Math.floor(lo) + 1; i <= Math.floor(hi); i++) {
        if (i > lo && i < hi && dist(pts[i], out[out.length - 1]) > 1e-9)
            out.push(pts[i]);
    }
    const tail = p0 <= p1 ? v1 : v0;
    if (dist(tail, out[out.length - 1]) > 1e-9)
        out.push(tail);
    else
 ...
 */
export function sampleEdgePolyline(t, curveId, v0, v1, sameSense, s, chordTol, maxSegLen, aRad = 1, normalDev = Math.PI) {
    ...
    if (bs && dist(deBoor(bs.degree, bs.cps, bs.knots, bs.u0, bs.weights), deBoor(bs.degree, bs.cps, bs.knots, bs.u1, bs.weights)) >= 1e-6) {
        ...
        for (let i = 1; i < cps.length; i++)
            clen += dist(cps[i - 1], cps[i]);
        const n = Math.max(2, Math.ceil(clen / Math.max(maxSegLen, 1e-9)));
        const pts = [];
        for (let i = 0; i <= n; i++)
            pts.push(deBoor(degree, cps, knots, u0 + ((u1 - u0) * i) / n, weights));
            ...
            if (ringClosed) {
                ...
                if (!(s0.seg === s1.seg && s0.t <= s1.t)) {
                    ...
                    for (let i = (s0.seg + 1) % ring.length;; i = (i + 1) % ring.length) {
                        const p = ring[i];
                        if (dist(p, arc[arc.length - 1]) > 1e-9)
                            arc.push(p);
                        if (i === s1.seg)
                            break;
                    }
                }
                if (dist(v1, arc[arc.length - 1]) > 1e-9)
                    arc.push(v1);
                else
                    arc[arc.length - 1] = v1;
                if (arc.length >= 2)
