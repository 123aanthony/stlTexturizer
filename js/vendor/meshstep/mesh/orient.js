...
export function orientConsistent(mesh, solidOfTri) {
    const idx = mesh.indices;
    const pos = mesh.positions;
    const nt = idx.length / 3;
    const ekey = (a, b) => (a < b ? a * KEY + b : b * KEY + a);
    ...
    const flip = (t) => { const i = t * 3 + 1, j = t * 3 + 2; const tmp = idx[i]; idx[i] = idx[j]; idx[j] = tmp; };
    const hasDir = (t, x, y) => {
        for (let e = 0; e < 3; e++)
            if (idx[t * 3 + e] === x && idx[t * 3 + ((e + 1) % 3)] === y)
                return true;
        return false;
    };
    ...
    for (let iter = 0; iter < 8; iter++) {
        ...
        for (let t = 0; t < nt; t++) {
            ...
            for (let e = 0; e < 3; e++) {
                ...
                if (inc.length !== 2)
                    continue; // open or non-manifold edge — no orientation signal
    ...
    for (const c of comps) {
        ...
        if (comps.length > 1 && solidOfTri) {
            for (const o of comps) {
                if (o === c || o.solid !== c.solid)
                    continue;
                const [x, y, z] = c.pt;
                if (x < o.box[0] || y < o.box[1] || z < o.box[2] || x > o.box[3] || y > o.box[4] || z > o.box[5])
                    continue;
                if (Math.abs(windingOf(o, c.pt)) > 0.75)
                    parity++;
            }
        }
        const wantNegative = (parity & 1) === 1;
        if (wantNegative ? c.vol > 0 : c.vol < 0)
            for (const t of c.tris)
                flip(t);
    }
}
