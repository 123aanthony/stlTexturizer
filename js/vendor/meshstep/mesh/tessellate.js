import { cross, dot, lerp, normalize } from "../geom/vec.js";
import { ref, refList, list, num, numList } from "../step/entities.js";
import { makeSurface, isSphere, isBSpline, Sphere } from "../geom/surfaces.js";
import { sampleEdgePolyline } from "../geom/curves.js";
import { constrainedTriangulate } from "./cdt2d.js";
import { earcut } from "./earcut.js";
import { ekey, refineTriangulation } from "./refine.js";
import { beginWarnings, takeWarnings, warn } from "./diag.js";
...
function emitTri(verts, faceIds, a, b, c, fid, surface, sign, refN) {
    const ng = cross([b[0] - a[0], b[1] - a[1], b[2] - a[2]], [c[0] - a[0], c[1] - a[1], c[2] - a[2]]);
    if (ng[0] * ng[0] + ng[1] * ng[1] + ng[2] * ng[2] < 1e-18)
...
function unwrap(p2, c, period = TWO_PI) {
    const half = period / 2;
...
function chainProject(surface, poly, hint) {
    const resid = (q, pt) => { const e = surface.evaluate(q[0], q[1]); return Math.hypot(e[0] - pt[0], e[1] - pt[1], e[2] - pt[2]); };
    const uP = surface.periodicU ? surface.uPeriod || TWO_PI : 0;
    const vP = surface.periodicV ? surface.vPeriod || TWO_PI : 0;
    const wrapd = (d, P) => { if (!P)
        return d; d %= P; if (d > P / 2)
        d -= P;
    else if (d < -P / 2)
        d += P; return d; };
    const p2 = [];
    let hu = hint?.[0], hv = hint?.[1];
    ...
    for (let i = 0; i < poly.length; i++) {
        ...
        if (i === 0) {
            ...
            const mstep = (c) => Math.hypot(wrapd(c[0] - prev[0], uP) * uScale, wrapd(c[1] - prev[1], vP) * vScale);
            const rq = resid(q, pt);
            if (rq > 1e-3 || mstep(q) > 3 * d3 + 1e-6) {
                const g = surface.project(pt);
                const rg = resid(g, pt);
                const qOk = rq <= 1e-3, gOk = rg <= 1e-3;
                if (gOk && (!qOk || mstep(g) < mstep(q)))
                    q = g;
                else if (!qOk && rg < rq)
                    q = g;
            }
...
function slitCollapse(poly, p2) {
    ...
    let k = 0, far = 0;
    for (let i = 0; i < n; i++) {
        const di = d3(poly[i], poly[0]);
        if (di > far) {
            far = di;
            k = i;
        }
    }
    if (k < 2 || k > n - 3 || far < 1e-9)
        return; // turn point must be interior
    if (d3(poly[0], poly[n - 1]) > 0.25 * far)
        return; // doesn't come back to its start
    const out = poly.slice(0, k + 1);
    const tol = Math.max(2e-3, 0.02 * far); // slit width: pinched, not a genuine thin U
    for (let i = k + 1; i < n; i++)
        if (distToPolyline(poly[i], out) > tol)
            return;
    for (let i = k + 1; i < n; i++) {
        let bj = 0, bd = Infinity;
        for (let j = 0; j <= k; j++) {
            const dd = d3(poly[i], poly[j]);
            if (dd < bd) {
                bd = dd;
                bj = j;
            }
...
function edgePcurve(surface, poly) {
    const p2 = chainProject(surface, poly);
    slitCollapse(poly, p2);
    if (surface.periodicU)
        unwrap(p2, 0, surface.uPeriod || TWO_PI);
    if (surface.periodicV)
        unwrap(p2, 1, surface.vPeriod || TWO_PI);
    return {
        p3: poly, p2,
...
function hugsSeam(surface, p2, c) {
    const periodic = c === 0 ? surface.periodicU : surface.periodicV;
    if (!periodic || p2.length === 0)
        return false;
    const period = (c === 0 ? surface.uPeriod : surface.vPeriod) || TWO_PI;
...
function assembleWire(surface, pcs, forced) {
    const uP = surface.periodicU ? surface.uPeriod || TWO_PI : 0;
    const vP = surface.periodicV ? surface.vPeriod || TWO_PI : 0;
    const p3 = [], p2 = [];
    ...
    for (let k = 0; k < pcs.length; k++) {
        ...
        if (k > 0) {
            if (uP)
                du = Math.round((eu - q0[0]) / uP) * uP;
            if (vP)
                dv = Math.round((ev - q0[1]) / vP) * vP;
        }
        if (forced) {
            du += forced[2 * k] * uP;
            dv += forced[2 * k + 1] * vP;
        ...
        const m = e.p2.length;
        for (let i = 0; i < m - 1; i++) {
            p2.push([e.p2[i][0] + du, e.p2[i][1] + dv]);
            p3.push(e.p3[i]);
        }
        eu = e.p2[m - 1][0] + du;
        ev = e.p2[m - 1][1] + dv;
...
function countSelfIntersections(pts, cap = 1 << 30, pairs) {
    const n = pts.length;
    if (n < 4)
        return 0;
    ...
    for (let i = 0; i < n; i++) {
        ...
        x0[i] = Math.min(a[0], b[0]);
        x1[i] = Math.max(a[0], b[0]);
        y0[i] = Math.min(a[1], b[1]);
        y1[i] = Math.max(a[1], b[1]);
    }
    ...
    for (let oi = 0; oi < n && count < cap; oi++) {
        ...
        for (let oj = oi + 1; oj < n; oj++) {
            const j = order[oj];
            if (x0[j] > x1[i])
                break; // sweep past i's extent — no later j can overlap
            if (y0[j] > y1[i] || y1[j] < y0[i])
                continue;
            if ((i + 1) % n === j || (j + 1) % n === i)
 ...
 */
function repairWire(surface, pcs, base, baseInts, expectSign, allowIso = false) {
    ...
    const slots = []; // flat (edgeIndex, axis) pairs, axis 0=u 1=v
    for (let k = 1; k < pcs.length; k++) {
        if (surface.periodicU && (pcs[k].hugU || (allowIso && pcs[k].isoU)))
            slots.push(2 * k);
        if (surface.periodicV && (pcs[k].hugV || (allowIso && pcs[k].isoV)))
            slots.push(2 * k + 1);
        if (slots.length >= 5)
            break; // 3^5 = 243 combinations — bounded
    }
    ...
    for (let combo = 1; combo < total; combo++) {
        ...
        if (sc[0] < bestScore[0] || (sc[0] === bestScore[0] && (sc[1] < bestScore[1]
            ...
            best = { asm, selfInts: ints };
            bestScore = sc;
        }
    }
    return best;
    function score(asm, ints) {
...
function legacyLoopParam(surface, loop, sampled) {
    const p3 = [];
    const p2 = [];
    ...
    if (!surface.periodicU && !surface.periodicV && p2.length >= 4) {
        const n = p2.length;
        const d2p = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1]);
        const d3p = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
        const ratios = [];
        for (let i = 0; i + 1 < n; i++) {
            const dd = d3p(p3[i], p3[i + 1]);
            if (dd > 1e-9)
                ratios.push(d2p(p2[i], p2[i + 1]) / dd);
        }
        ratios.sort((a, b) => a - b);
        ...
        if (med > 0 && jump > 20 * med * Math.max(d3p(p3[n - 1], p3[0]), 1e-9) && jump > 0.05 * extent) {
            ...
            for (let i = 0; i < n >> 1; i++) {
                ...
                if (d2p(q, p2[i]) < 1e-6 * Math.max(extent, 1e-9))
                    break; // rejoined the original chain
                const eOld = surface.evaluate(p2[i][0], p2[i][1]);
                const eNew = surface.evaluate(q[0], q[1]);
                const rOld = d3p(eOld, p3[i]), rNew = d3p(eNew, p3[i]);
                if (rNew > Math.max(2 * rOld, 1e-3))
                    break; // redo left the surface — keep the original
                p2[i] = q;
                hint = q;
            }
        }
    ...
    if (n >= 4) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (dAB < 8 * dAC + 1e-9 || dBC < 8 * dAC + 1e-9)
                continue;
            const q = surface.project(p3[i], a[0], a[1]);
            if (d2(q, a) + d2(q, c) >= 0.5 * (dAB + dBC))
                continue; // did not land between the neighbours
...
function cycleWind(p2, c, period) {
    if (!period)
        return 0;
    let w = 0;
    const half = period / 2;
...
export function loopParam(surface, loop, sampled, expectSign = 0) {
    const uP = surface.periodicU ? surface.uPeriod || TWO_PI : 0;
    const vP = surface.periodicV ? surface.vPeriod || TWO_PI : 0;
    const leg = legacyLoopParam(surface, loop, sampled);
    if ((!uP && !vP) || leg.p2.length < 4)
        return { ...leg, windU: 0, windV: 0, tangled: false };
    const legInts = countSelfIntersections(leg.p2);
    const legWind = [cycleWind(leg.p2, 0, uP), cycleWind(leg.p2, 1, vP)];
    ...
    if (legInts === 0 && !collapsedLoop(leg.p2))
        return { ...leg, windU: legWind[0], windV: legWind[1], tangled: false };
    const pcs = [];
    for (const oe of loop.edges) {
        const base = sampled.get(oe.edgeId);
        if (!base)
    ...
    if (pcs.length === 0)
        return { ...leg, windU: legWind[0], windV: legWind[1], tangled: true };
    let asm = assembleWire(surface, pcs);
    let selfInts = countSelfIntersections(asm.p2);
    const asmCollapsed = collapsedLoop(asm.p2);
    if (selfInts > 0 || asmCollapsed)
        ({ asm, selfInts } = repairWire(surface, pcs, asm, selfInts, expectSign, asmCollapsed));
...
function shiftIntoRange(p2, target, c, period = TWO_PI) {
    unwrap(p2, c, period);
    let mean = 0;
    for (const p of p2)
        mean += p[c];
...
function faceTarget(surface, targetEdge, chordTol, normalDev, u = 0, v = 0, floorAngular = false) {
 ...
 */
function tessellateParamGrid(surface, loops, sampled, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign) {
    ...
    if (oi < 0) {
        ...
        for (let i = 0; i < projected.length; i++) {
            const e = bboxExtent(projected[i].lp.p2);
            if (e > best) {
                best = e;
                oi = i;
            }
    ...
    if (outer.p3.length < 3) {
        if (DBG)
            console.error(`[grid] fid=${fid} bail: outer loop projects to ${outer.p3.length} pts`);
        return false;
    }
    ...
    // anisotropy fine once it's allowed through.
    {
        ...
        // measures it exactly.
        const arc = (dir) => {
            const span = Math.min(dir === 0 ? du : dv, (dir === 0 ? surface.periodicU : surface.periodicV) ? ((dir === 0 ? surface.uPeriod : surface.vPeriod) || TWO_PI) : Infinity);
            let s = 0;
        ...
        if (2 * areaM / Math.max(perM, 1e-12) < Math.max(1e-3, 1e-9 * extS)) {
            if (DBG)
                console.error(`[grid] fid=${fid} bail: metric width ${(2 * areaM / Math.max(perM, 1e-12)).toExponential(2)} (area=${areaM.toExponential(2)} per=${perM.toFixed(3)} ext=${extS.toFixed(3)})`);
            return false;
        }
    }
    ...
    if (surface.periodicU)
        for (const h of holes)
            shiftIntoRange(h.p2, (umin + umax) / 2, 0, surface.uPeriod || TWO_PI);
    if (surface.periodicV)
        for (const h of holes)
            shiftIntoRange(h.p2, (vmin + vmax) / 2, 1, surface.vPeriod || TWO_PI);
    ...
    if (!gridCDT(surface, outer, holes, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign, clean, false, false, outerWinds))
        return false;
    ...
    if (overCovered()) {
        ...
        if (!gridCDT(surface, outer, holes, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign, clean, true, false, outerWinds) || overCovered()) {
            ...
            if (gridCDT(surface, outer, holes, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign, clean, true, true, outerWinds) && !overCovered()) {
                if (DBG)
                    console.error(`[grid] fid=${fid} FOLD AUDIT: de-slit retry is fold-free`);
                return true;
            }
            verts.length = v0;
            faceIds.length = f0;
            ...
            if (surface.kind === "PLANE" && planeEarcutFill(surface, outer, holes, fid, verts, faceIds, sign, targetEdge)) {
                ...
                if (DBG)
                    console.error(`[grid] fid=${fid} FOLD AUDIT: earcut hole-bridge fill (structural multi-loop plane)`);
                return true;
            }
            if (DBG)
                console.error(`[grid] fid=${fid} FOLD AUDIT rollback (boundary segment over-covered)`);
            verts.length = v0;
            faceIds.length = f0;
            return false;
        }
        if (DBG)
            console.error(`[grid] fid=${fid} FOLD AUDIT: un-refined retry is fold-free`);
    }
        ...
        while (over > 0 && guard++ < 256) {
            ...
            for (const [k, m] of all)
                if (m > 2 && (bound.get(k) ?? 0) < m)
                    over++;
            if (DBG && over === 0)
                console.error(`[grid] fid=${fid} self-overlap surgery: dropped ${guard} folded triangle(s)`);
        }
        if (peeled > 0)
            warn("folded-triangles-dropped", fid, `folded (u,v) projection — ${peeled} redundant triangle(s) peeled off the face`);
    }
    return true;
}
...
function planeEarcutFill(surface, outer, holes, fid, verts, faceIds, sign, targetEdge) {
    const flat = [];
    const p3 = [];
    for (let i = 0; i < outer.p2.length; i++) {
        flat.push(outer.p2[i][0], outer.p2[i][1]);
        p3.push(outer.p3[i]);
    }
    const holeStarts = [];
    for (const h of holes) {
        if (h.p2.length < 3)
            continue; // a degenerate hole contributes no area — skip it, keep the rest
        holeStarts.push(flat.length / 2);
        for (let i = 0; i < h.p2.length; i++) {
            flat.push(h.p2[i][0], h.p2[i][1]);
            p3.push(h.p3[i]);
        }
    }
    ...
    const widx = new Map();
    const remap = new Array(flat.length / 2);
    const wpts = [];
    const wp3 = [];
    ...
    const pt3 = (i) => (i < nW ? wp3[i] : surface.evaluate(wpts[i * 2], wpts[i * 2 + 1]));
...
function gridCDT(surface, outer, holes, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign, allowAniso = true, noRefine = false, forceDeslit = false, requireClosed = false) {
    let umin = Infinity, umax = -Infinity, vmin = Infinity, vmax = -Infinity;
    for (const q of outer.p2) {
        if (q[0] < umin)
            umin = q[0];
    ...
    for (const fu of [0.15, 0.5, 0.85])
        for (const fv of [0.15, 0.5, 0.85]) {
            ...
            const d1u = [(uPp[0] - uPm[0]) / (2 * epsU), (uPp[1] - uPm[1]) / (2 * epsU), (uPp[2] - uPm[2]) / (2 * epsU)];
            const d1v = [(vPp[0] - vPm[0]) / (2 * epsV), (vPp[1] - vPm[1]) / (2 * epsV), (vPp[2] - vPm[2]) / (2 * epsV)];
            uMags.push(Math.hypot(d1u[0], d1u[1], d1u[2]));
            vMags.push(Math.hypot(d1v[0], d1v[1], d1v[2]));
            const nrm = normalize(cross(d1u, d1v));
            const normCurv = (Pp, Pn, d1, e) => {
                const d2 = [(Pp[0] + Pn[0] - 2 * Pc[0]) / (e * e), (Pp[1] + Pn[1] - 2 * Pc[1]) / (e * e), (Pp[2] + Pn[2] - 2 * Pc[2]) / (e * e)];
    ...
    const sanitize = (lp) => {
        ...
        for (let i = 0; i < n; i++) {
            const k = ringKey(i);
            while (keep.length >= 2 && ringKey(keep[keep.length - 2]) === k)
                keep.pop();
            if (keep.length >= 1 && ringKey(keep[keep.length - 1]) === k)
                continue;
            keep.push(i);
        }
        ...
        for (let pass = 0; pass < 4; pass++) {
            ...
            for (let i = 0; i < m && !cut; i++) {
                ...
                if (r < 2)
                    continue;
                let L = 0;
                while (L < r && i + L < m && kOf[j + L] === kOf[i + L])
                    L++;
                if (L >= Math.max(2, r - 1))
                    cut = [j, i]; // drop one full period [j, i)
            }
            if (!cut)
                break;
    ...
    const inRegion = (p) => pointInPoly(p, outer.p2) && !holeP2.some((h) => pointInPoly(p, h));
    ...
    const hkey = (ix, iy) => Math.imul(ix, 73856093) ^ Math.imul(iy, 19349663);
    const segHash = new Map();
    for (let i = 0; i < sseg.length; i++) {
        const k = hkey(Math.floor((sseg[i][0] + sseg[i][2]) / 2 / csz), Math.floor((sseg[i][1] + sseg[i][3]) / 2 / csz));
        (segHash.get(k) ?? segHash.set(k, []).get(k)).push(i);
    }
    const floor = target / 40;
    const sizeDist = (sx, sy) => {
        ...
        if (!(Math.abs(sx) < 1e12 && Math.abs(sy) < 1e12))
        ...
        for (let gx = cx - 2; gx <= cx + 2; gx++)
            for (let gy = cy - 2; gy <= cy + 2; gy++) {
                const arr = segHash.get(hkey(gx, gy));
                if (!arr)
                    continue;
                for (const i of arr) {
                    const [ax, ay, bx, by, ln] = sseg[i], ex = bx - ax, ey = by - ay, l2 = ex * ex + ey * ey;
                    let tt = l2 > 0 ? ((sx - ax) * ex + (sy - ay) * ey) / l2 : 0;
                    tt = tt < 0 ? 0 : tt > 1 ? 1 : tt;
    ...
    const tryAdd = (u, v) => {
        ...
        if (dist < 0.3 * sz)
            return; // hugs a boundary edge -> would make it un-enforceable
        interiorIdx.push(allP2.length);
        allP2.push([u, v]);
        allP3.push(surface.evaluate(u, v));
    };
    for (let i = 1; i < nU; i++)
        for (let j = 1; j < nV; j++)
            tryAdd(umin + ((umax - umin) * i) / nU, vmin + ((vmax - vmin) * j) / nV);
    const cdtPts = allP2.map(([u, v]) => [SX(u), SY(v)]);
    ...
    const altHoles = [];
    {
        const key3 = (v) => `${allP3[v][0]},${allP3[v][1]},${allP3[v][2]}`;
        const near2i = (a, b) => Math.abs(cdtPts[a][0] - cdtPts[b][0]) <= 1e-6 && Math.abs(cdtPts[a][1] - cdtPts[b][1]) <= 1e-6;
        const inPoly = (p, poly) => {
            let inside = false;
            for (let i = 0, j = poly.length - 1; i < poly.length; j = i++) {
        ...
        for (let guard = 0; guard < 8; guard++) {
            ...
            for (let jj = 0; jj < n && done; jj++) {
                ...
                const farOf = (p, at) => cdtPts[p[(at + (p.length >> 1)) % p.length]];
                let outer, hole;
                if (inPoly(farOf(mid, 0), wrap)) {
                    outer = wrap;
                    hole = mid;
                }
                else if (inPoly(farOf(wrap, i), mid)) {
                    outer = mid;
                    hole = wrap;
                }
                else
                    continue; // neither encloses the other — leave alone
                if (DBG)
                    console.error(`[deslit] fid=${fid} candidate corridor (${i},${j}) m=${m} -> outer=${outer.length} hole=${hole.length}`);
                altHoles.push(hole);
                cur = outer;
    ...
    const weldPinches = (idx) => {
        const n = idx.length;
        if (n < 4)
            return;
        const seen = new Map(); // exact (u,v)+3D key -> position of first occurrence
        for (let i = 0; i < n; i++) {
            const v = idx[i];
            ...
            const co = (x, y) => cdtPts[idx[x]][0] === cdtPts[idx[y]][0] && cdtPts[idx[x]][1] === cdtPts[idx[y]][1];
    ...
    for (const idx of [outerIdx, ...holeIdx])
        weldPinches(idx);
    if (altOuter) {
        weldPinches(altOuter);
        for (const h of altHoles)
            weldPinches(h);
    }
    ...
    if (altOuter) {
        ...
        const coverDefects = (T, loopsSet) => {
            const key = (x, y) => { const a = g(x), b = g(y); return a < b ? a * 0x8000000 + b : b * 0x8000000 + a; };
            const have = new Map();
            for (const [x, y, z] of T)
                for (const k of [key(x, y), key(y, z), key(z, x)])
                    have.set(k, (have.get(k) ?? 0) + 1);
            const want = new Map();
            for (const lp of loopsSet)
        ...
        if (d0 > 0) {
            ...
            if (DBG)
                console.error(`[deslit] fid=${fid} outcome: original defects=${d0}, de-slit defects=${d1} -> ${d1 === 0 && d1 < d0 ? "ADOPT" : "keep original"}`);
            if (d1 === 0 && d1 < d0) {
                outerIdx.length = 0;
    ...
    if (DBG)
        console.error(`[grid] fid=${fid} nU=${nU} nV=${nV} uSc=${uSc.toExponential(2)} vSc=${vSc.toExponential(2)} boundary=${outerIdx.length}+${holeIdx.reduce((s, h) => s + h.length, 0)} interior=${interiorIdx.length} -> cdt tris=${tris.length}${cdtOut.missing ? ` MISSING=${cdtOut.missing}` : ""}`);
    ...
    const dupKey = (x, y) => hkey(Math.round(x / dupCell), Math.round(y / dupCell));
    const dupSeen = new Set();
    const dupHas = (x, y) => {
        const cx = Math.round(x / dupCell), cy = Math.round(y / dupCell);
        for (let gx = cx - 1; gx <= cx + 1; gx++)
            for (let gy = cy - 1; gy <= cy + 1; gy++) {
                if (dupSeen.has(hkey(gx, gy)))
                    return true;
            }
        return false;
    };
    for (const p of cdtPts)
        dupSeen.add(dupKey(p[0], p[1]));
    for (let iter = 0; iter < maxIter && interiorIdx.length < cap; iter++) {
        const fresh = []; // (px, py, local size) — sizeDist carried to the dedup pass
        for (const [a, b, c] of tris) {
            ...
            if (!(Math.abs(px) < 1e12 && Math.abs(py) < 1e12))
                continue;
        ...
        for (const [px, py, sz] of fresh) {
            ...
            let ok = true;
            for (let gx = hx - 1; gx <= hx + 1 && ok; gx++)
                for (let gy = hy - 1; gy <= hy + 1 && ok; gy++) {
                    for (const [qx, qy] of acc.get(hkey(gx, gy)) ?? [])
                        if ((px - qx) ** 2 + (py - qy) ** 2 < (0.5 * sz) ** 2)
                            ok = false;
                }
            if (!ok)
                continue;
            ...
            interiorIdx.push(allP2.length);
            allP2.push([px / uSc, py / vSc]);
            allP3.push(surface.evaluate(px / uSc, py / vSc));
            cdtPts.push([px, py]);
            added = true;
        }
        if (!added)
        ...
        if (cdtOut.missing > prevMissing) {
            if (DBG)
                console.error(`[grid] fid=${fid} refinement iter ${iter} lost ${cdtOut.missing - prevMissing} constraint(s) — rolled back`);
            tris = prevTris;
            break;
        }
    ...
    if (requireClosed && prevMissing > 0) {
        if (DBG)
            console.error(`[grid] fid=${fid} requireClosed: ${prevMissing} constraint(s) unrealised — face declined`);
        return false;
    }
    ...
    if (prevMissing > 0)
        warn("cdt-degenerate-boundary", fid, `${prevMissing} boundary constraint(s) unenforceable — region rebuilt by rescue fill`);
    else if (prevRescue)
        warn("cdt-degenerate-boundary", fid, `degenerate boundary (duplicate/collinear vertices) — region rebuilt by ${prevRescue} fill`);
    ...
    const ekey = (i, j) => (i < j ? i * NPTS + j : j * NPTS + i);
    const constrained = new Set();
    for (const idx of [outerIdx, ...holeIdx])
        for (let i = 0; i < idx.length; i++)
            constrained.add(ekey(idx[i], idx[(i + 1) % idx.length]));
    ...
    const ringOf = new Int32Array(nBoundary), ringPos = new Int32Array(nBoundary), ringLen = [];
    {
        let r = 0;
        for (const idx of [outerIdx, ...holeIdx]) {
            for (let i = 0; i < idx.length; i++) {
            ...
            ringLen.push(idx.length);
            r++;
        }
    }
    const boundaryShortcut = (i, j) => {
        if (i >= nBoundary || j >= nBoundary)
            return false;
    ...
    for (let pass = 0; pass < 6; pass++) {
        const use = new Map(); // edge -> [triIndex, oppositeVertex]
        ...
        for (const [k, ts] of use) {
            if (ts.length !== 2 || constrained.has(k))
                continue;
            const [[t1, c1], [t2, c2]] = ts;
            if (dirty.has(t1) || dirty.has(t2))
                continue;
            if (boundaryShortcut(c1, c2))
                continue;
            ...
            if (f1[3] < 1e-6 * (o1[3] + o2[3]) || f2[3] < 1e-6 * (o1[3] + o2[3]))
                continue;
            if (f1[0] * o1[0] + f1[1] * o1[1] + f1[2] * o1[2] <= 0 || f2[0] * o2[0] + f2[1] * o2[1] + f2[2] * o2[2] <= 0)
                continue;
            if (f1[0] * o2[0] + f1[1] * o2[1] + f1[2] * o2[2] <= 0 || f2[0] * o1[0] + f2[1] * o1[1] + f2[2] * o1[2] <= 0)
                continue;
            ...
            const dotN = (p, q) => p ? p[0] * q[0] + p[1] * q[1] + p[2] * q[2] : 0;
    ...
    if (DBG)
        console.error(`[grid] fid=${fid} final tris=${tris.length} pts=${cdtPts.length}`);
 ...
 */
function tessellatePeriodicUnroll(surface, loops, sampled, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign) {
    const collect = (lp) => {
        const p3 = [];
    ...
    const attempt = (c) => {
        ...
        const proj = loops.map((lp) => {
            ...
            let hu, hv;
            for (const pt of p3) {
                const q = surface.project(pt, hu, hv);
                p2.push(q);
                hu = q[0];
                hv = q[1];
            }
        ...
        if (proj.some((p) => p === null))
            return false;
        const rims = proj.filter((p) => Math.abs(p.wind) >= 0.9 * period);
        const holes = proj.filter((p) => Math.abs(p.wind) < 0.9 * period);
        if (rims.length !== 2)
            return false;
        const meanStack = (p) => { let s = 0; for (const q of p.p2)
            s += q[stackC]; return s / p.p2.length; };
        ...
        const norm = (x) => ((x % period) + period) % period;
        let seam;
        const angs = [];
        for (const h of holes)
            for (const q of h.p2)
        ...
        const normTo = (x) => { let d = (x - seam) % period; if (d < 0)
            d += period; return seam + d; };
        const toP2 = (a, s) => (c === 0 ? [a, s] : [s, a]);
        ...
        const buildRim = (rim, atSeam = seam) => {
            ...
            const outP3 = [p3[k]];
            const auv = [[atSeam + best, p2[k][stackC]]];
            for (let i = 1; i < n; i++) {
                ...
                while (d < -period / 2)
                    d += period;
                outP3.push(p3[j]);
                auv.push([auv[i - 1][0] + d, p2[j][stackC]]);
            }
            outP3.push(p3[k]);
            auv.push([auv[0][0] + period, auv[0][1]]);
        ...
        const conflicted = () => {
            if (countSelfIntersections(outer.p2) > 0)
                return true;
            const o2 = (a, b, q) => (b[0] - a[0]) * (q[1] - a[1]) - (b[1] - a[1]) * (q[0] - a[0]);
            const sx = (p, q, r, s) => {
            ...
            for (const h of holeLoops) {
                for (let i = 0; i < h.p2.length; i++) {
                    ...
                    for (let j = 0; j < on; j++) {
                        if (sx(a, b, outer.p2[j], outer.p2[(j + 1) % on]))
                            return true;
                    }
                }
            }
        ...
        // original (u,v): the accumulator unwraps in a-space and each emitted point adds s·stack back.
        {
            const cross = (p2, uk, slope = 0) => {
                ...
                for (let i = 0; i < p2.length; i++) {
                    ...
                    if (Math.abs(d) < 1e-12)
                        continue;
                    const r = norm(uk - a);
                    if ((d > 0 && r > 0 && r < d) || (d < 0 && r - period > d && r < period))
                        cnt++;
                }
                return cnt;
            };
            const buildRimCut = (rim, uStar, slope = 0) => {
                let p3 = rim.p3, p2 = rim.p2;
                if (rim.wind < 0) {
                    p3 = p3.slice().reverse();
                    p2 = p2.slice().reverse();
                ...
                const n = p2.length;
                const wrapD = (d) => { while (d > period / 2)
                    d -= period; while (d < -period / 2)
                    d += period; return d; };
                const aOf = (q) => q[c] - slope * q[stackC];
                let ci = -1, ct = 0, cs = 0;
                for (let i = 0; i < n; i++) {
                    ...
                    if (d > 0 && r > 0 && r < d)
                        t = r / d;
                    else if (d < 0 && r - period > d && r < period)
                        t = (r - period) / d;
                    if (t > 0 && t < 1) {
                        if (ci >= 0)
                            return null; // crosses more than once — the scan should have excluded uStar
                        ci = i;
                        ct = t;
                ...
                const outP3 = [cut3];
                const auv = [[uStar + slope * cs, cs]];
                let acc = uStar + (1 - ct) * wrapD(aOf(p2[j]) - aOf(p2[ci]));
                outP3.push(p3[j]);
                auv.push([acc + slope * p2[j][stackC], p2[j][stackC]]);
                for (let k = 1; k < n; k++) {
                    const idx = (j + k) % n, prv = (j + k - 1) % n;
                    acc += wrapD(aOf(p2[idx]) - aOf(p2[prv]));
                    outP3.push(p3[idx]);
                    auv.push([acc + slope * p2[idx][stackC], p2[idx][stackC]]);
                }
                outP3.push(cut3);
                auv.push([uStar + period + slope * cs, cs]);
                return { p3: outP3, p2: auv.map(([a, s]) => toP2(a, s)) };
            };
            const K = 512;
            if (conflicted()) {
                ...
                for (let k = 0; k < K; k++) {
                    ...
                    if (cross(rims[0].p2, uk) !== 1 || cross(rims[1].p2, uk) !== 1)
                        continue;
                    let ok = true;
                    for (const h of holes)
                        if (cross(h.p2, uk) > 0) {
                    ...
                    for (const arr of [rims[0].p2, rims[1].p2, ...holes.map((h) => h.p2)]) {
                        for (const q of arr) {
                            const dd = norm(q[c] - uk);
                            const m = Math.min(dd, period - dd);
                            if (m < margin)
                                margin = m;
                        }
                    }
                ...
                if (DBG && !Number.isFinite(uStar))
                    console.error(`[unroll] fid=${fid} exact-seam: no meridian crosses each rim exactly once (weaving rims)`);
                if (Number.isFinite(uStar)) {
                    const b2 = buildRimCut(rims[0], uStar), t2 = buildRimCut(rims[1], uStar);
                    if (b2 && t2) {
                        const normTo2 = (x) => { let d = (x - uStar) % period; if (d < 0)
                            d += period; return uStar + d; };
                        ...
                        if (DBG)
                            console.error(`[unroll] fid=${fid} exact-seam rebuild at ${uStar.toFixed(4)} (margin ${bestMargin.toExponential(1)})`);
                    }
                }
            }
            ...
            const foldedHole = () => holeLoops.some((h) => {
                for (let i = 0; i < h.p2.length; i++) {
                    if (Math.abs(h.p2[(i + 1) % h.p2.length][c] - h.p2[i][c]) > period / 2)
                        return true;
                }
                return false;
            });
            ...
            if (conflicted()) {
                ...
                for (let k = 0; k < K; k++) {
                    ...
                    const normToK = (x) => { let d = (x - seamK) % period; if (d < 0)
                        d += period; return seamK + d; };
                    holeLoops = holes.map((h) => ({ p3: h.p3, p2: h.p2.map((q) => toP2(normToK(q[c]), q[stackC])) }));
                    if (!conflicted() && (wasFolded || !foldedHole())) {
                        if (DBG)
                            console.error(`[unroll] fid=${fid} vertex-cut seam retry: adopted seam=${seamK.toFixed(4)} (k=${k})`);
                        break;
                    }
                    outer = prevOuter;
                    holeLoops = prevHoles;
            ...
            if (holes.length && (conflicted() || foldedHole() || holeLoops.some((h) => countSelfIntersections(h.p2) > 0))) {
                ...
                const unw = holes.map((h) => {
                    const a = [norm(h.p2[0][c])];
                    for (let i = 1; i < h.p2.length; i++)
                        a.push(a[i - 1] + wrapD(h.p2[i][c] - h.p2[i - 1][c]));
                    return a;
                });
                const slopes = [];
                for (let hi = 0; hi < holes.length; hi++) {
                    ...
                    if (mx - mn < 0.95 * period)
                        continue; // fits a vertical cut — not a slope driver
                    ...
                    let num = 0, den = 0;
                    for (let i = 0; i < a.length; i++) {
                        const ds = hp[i][stackC] - sm;
                        num += ds * (a[i] - am);
                        den += ds * ds;
                    }
                    if (den > 1e-18 && Math.abs(num / den) > 1e-12)
                        slopes.push(num / den);
                }
                slopes.sort((x, y) => x - y);
                const cands = [0];
                if (slopes.length)
                    cands.push(slopes[slopes.length >> 1]);
                for (const s of slopes)
                    if (!cands.some((x) => Math.abs(x - s) <= 1e-9 * Math.max(1, Math.abs(s))))
                        cands.push(s);
                if (DBG)
                    console.error(`[unroll] fid=${fid} seam-path rescue: ${slopes.length} winding hole(s), slope candidates=[${cands.map((s) => s.toExponential(2)).join(",")}]`);
                for (const slope of cands) {
                    const aOf = (q) => q[c] - slope * q[stackC];
                    let aStar = NaN, aMargin = -1;
                    for (let k = 0; k < K; k++) {
                        const ak = ((k + 0.5) * period) / K;
                        if (cross(rims[0].p2, ak, slope) !== 1 || cross(rims[1].p2, ak, slope) !== 1)
                            continue;
                        let ok = true;
                        for (const h of holes)
                            if (cross(h.p2, ak, slope) > 0) {
                                ok = false;
                                break;
                            }
                        if (!ok)
                    ...
                    if (!Number.isFinite(aStar)) {
                        if (DBG)
                            console.error(`[unroll] fid=${fid} seam-path: no clean meridian at slope=${slope.toExponential(2)}`);
                        continue;
                    }
                    ...
                    if (!b2 || !t2) {
                        if (DBG)
                            console.error(`[unroll] fid=${fid} seam-path: rim cut failed at a*=${aStar.toFixed(4)} slope=${slope.toExponential(2)}`);
                        continue;
                    }
                    ...
                    for (let i = 1; i < nSeam; i++) {
                        ...
                        const q = toP2(aStar + slope * s, s);
                        seam3.push(surface.evaluate(q[0], q[1]));
                        seamS.push(s);
                    }
                    const prevOuter = outer, prevHoles = holeLoops;
                    outer = {
                    ...
                    holeLoops = holes.map((h, hi) => {
                        ...
                        let bMin = Infinity;
                        for (let i = 0; i < a.length; i++) {
                            const b = a[i] - slope * h.p2[i][stackC];
                            if (b < bMin)
                                bMin = b;
                        }
                    ...
                    if (conflicted() || foldedHole()) {
                        if (DBG)
                            console.error(`[unroll] fid=${fid} seam-path: rebuild at slope=${slope.toExponential(2)} still conflicted — reverted`);
                        outer = prevOuter;
                        holeLoops = prevHoles;
                        continue;
                    }
                    if (DBG)
                        console.error(`[unroll] fid=${fid} seam-path rebuild slope=${slope.toExponential(2)} a*=${aStar.toFixed(4)} seamPts=${nSeam - 1} (margin ${aMargin.toExponential(1)})`);
                    break;
                }
            }
        }
 ...
 */
function stitchRings(verts, faceIds, A, B, fid, surface, sign, fracA, fracB) {
    const na = A.length, nb = B.length;
    if (na === 1) {
        for (let j = 0; j < nb; j++)
            emitTri(verts, faceIds, A[0], B[j], B[(j + 1) % nb], fid, surface, sign);
        return;
    }
    if (nb === 1) {
        for (let k = 0; k < na; k++)
            emitTri(verts, faceIds, A[k], A[(k + 1) % na], B[0], fid, surface, sign);
        return;
    }
    const fa = (i) => (i >= na ? 1 : fracA ? fracA[i] : i / na);
    const fb = (i) => (i >= nb ? 1 : fracB ? fracB[i] : i / nb);
    let ia = 0, ib = 0;
    while (ia < na || ib < nb) {
        if (ia < na && (ib >= nb || fa(ia) < fb(ib))) {
            emitTri(verts, faceIds, A[ia % na], A[(ia + 1) % na], B[ib % nb], fid, surface, sign);
            ia++;
        }
        else {
            emitTri(verts, faceIds, A[ia % na], B[(ib + 1) % nb], B[ib % nb], fid, surface, sign);
            ib++;
        }
    }
}
 ...
 */
function reorientSphere(s, loops, sampled) {
    const c = s.f.o;
    const dirs = [];
    let mx = 0, my = 0, mz = 0;
    ...
    if (dirs.length < 3 || mL < 1e-9 * dirs.length)
        return s; // no privileged centre (band-like patch)
    const m = [mx / mL, my / mL, mz / mL];
    let minDot = 1;
    for (const d of dirs)
        minDot = Math.min(minDot, dot(d, m));
    if (minDot < Math.cos(1.31))
        return s; // a point sits > ~75° from the centre — can't clear the poles
    const up = Math.abs(m[0]) < 0.9 ? [1, 0, 0] : [0, 1, 0];
 ...
 */
function tessellateRevolutionBand(surface, loops, sampled, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign) {
    const d3 = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
    ...
    const bandAlong = (c) => {
        ...
        for (const lp of loops) {
            ...
            for (const p of pts) {
                ...
                hcos += Math.cos((h * TWO_PI) / stackPeriod);
                hsin += Math.sin((h * TWO_PI) / stackPeriod);
                if (a < amin)
                    amin = a;
                if (a > amax)
                    amax = a;
            ...
            for (let i = 0; i < as.length; i++) {
                ...
                if (stackPeriodic) {
                    while (dh > stackPeriod / 2)
                        dh -= stackPeriod;
                    while (dh < -stackPeriod / 2)
                        dh += stackPeriod;
                }
                if (Math.abs(d) < 1e-8 && Math.abs(dh) > 1e-8)
                    return false;
            }
            if (Math.abs(wind) < 0.9 * period && amax - amin < 0.9 * period)
                return false; // partial arc
            ...
            if (!stackPeriodic && absWind > Math.abs(wind) + 0.05 * period)
                return false;
        ...
        if (!stackPeriodic) {
            ...
            if (width <= 1e-9)
                width += stackPeriod;
            if (width >= stackPeriod - 1e-9)
                return false; // degenerate: rims coincide in stack coordinate
            stack.push({ from: A.h, width, bottom: A.pts, top: B.pts, bWind: A.wind, tWind: B.wind });
        }
        const half = period / 2;
        const wrap = (d) => { while (d > half)
            d -= period; while (d < -half)
            d += period; return d; };
        ...
        const align = (ref, ring, refWind = 0, ringWind = 0) => {
            ...
            for (let i = 0; i < r.length; i++) {
                const dd = Math.abs(wrap(angleOf(r[i]) - a0));
                if (dd < bd) {
                    bd = dd;
                    bi = i;
                }
        ...
        for (const band of stack) {
            ...
            const hOf = (p) => surface.project(p)[1 - c];
            const nearTo = (h, ref) => {
                if (!stackPeriodic)
                    return h;
                while (h - ref > stackPeriod / 2)
                    h -= stackPeriod;
                while (h - ref < -stackPeriod / 2)
                    h += stackPeriod;
                return h;
            };
            const progressOf = (pts) => {
                const out = [0];
                for (let i = 1; i < pts.length; i++)
                    out.push(out[i - 1] + Math.abs(wrap(angleOf(pts[i]) - angleOf(pts[i - 1]))));
                return out;
            };
            ...
            const hTopAt = (s) => {
                ...
                while (ti > 0 && progT[ti] > s)
                    ti--;
                while (ti + 1 < progT.length && progT[ti + 1] < s)
                    ti++;
                if (ti + 1 >= progT.length)
 ...
 */
function tessellateThinFace(surface, outerLoop, sampled, fid, verts, faceIds, sign, tol) {
    const p = [];
    ...
    const far = (ox, oy, oz) => {
        let bi = 0, bd = -1;
        for (let i = 0; i < m; i++) {
            const d = (p[i][0] - ox) ** 2 + (p[i][1] - oy) ** 2 + (p[i][2] - oz) ** 2;
            if (d > bd) {
                bd = d;
                bi = i;
            }
    ...
    for (let i = 0; i < na; i += step1)
        if (distToPolyline(c1[i], c2) > tol)
            return false;
    for (let j = 0; j < nb; j += step2)
        if (distToPolyline(c2[j], c1) > tol)
            return false;
    // Stitch the two rails into a ribbon, advancing whichever is behind in arc-length fraction.
    let i = 0, j = 0;
    while (i < na - 1 || j < nb - 1) {
        if (j >= nb - 1 || (i < na - 1 && (i + 1) / na <= (j + 1) / nb)) {
            emitTri(verts, faceIds, c1[i], c1[i + 1], c2[j], fid, surface, sign);
            i++;
        }
        else {
            emitTri(verts, faceIds, c1[i], c2[j + 1], c2[j], fid, surface, sign);
            j++;
        }
    }
    return true;
 ...
 */
function tessellateSliverLoop(surface, outerLoop, sampled, fid, verts, faceIds, sign, chordTol) {
    const p = [];
    for (const oe of outerLoop.edges) {
        const base = sampled.get(oe.edgeId);
        if (!base)
            continue; // healed-away micro-edge: ring closes without it
        const poly = oe.orient ? base : base.slice().reverse();
        for (let i = 0; i < poly.length - 1; i++)
            p.push(poly[i]);
    }
    ...
    if (width > Math.max(1e-3, 2 * chordTol)) {
        if (DBG)
            console.error(`[tess] fid=${fid} sliverLoop bail: 3D width ${width.toExponential(2)} > ${Math.max(1e-3, 2 * chordTol).toExponential(2)}`);
        return false;
    }
    const far = (ox, oy, oz) => {
        let bi = 0, bd = -1;
    ...
    for (let i = 0; i < na; i += step1) {
        ...
        if (d > dtol) {
            if (DBG)
                console.error(`[tess] fid=${fid} sliverLoop bail: rail1[${i}] ${d.toExponential(2)} > dtol ${dtol.toExponential(2)}`);
            return false;
        }
    }
    for (let j = 0; j < nb; j += step2) {
        ...
        if (d > dtol) {
            if (DBG)
                console.error(`[tess] fid=${fid} sliverLoop bail: rail2[${j}] ${d.toExponential(2)} > dtol ${dtol.toExponential(2)}`);
            return false;
        }
    }
    if (DBG)
        console.error(`[tess] fid=${fid} sliverLoop: width=${width.toExponential(2)} rails=${na}/${nb}`);
    let i = 0, j = 0;
    while (i < na - 1 || j < nb - 1) {
        if (j >= nb - 1 || (i < na - 1 && (i + 1) / na <= (j + 1) / nb)) {
            emitTri(verts, faceIds, c1[i], c1[i + 1], c2[j], fid, surface, sign);
 ...
 */
function tessellateThinStrip(surface, outerLoop, sampled, fid, verts, faceIds, sign, tol) {
    ...
    if (countSelfIntersections(p2) === 0)
        return false;
    const d3 = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
    let per = 0;
    for (let i = 0; i < m; i++)
    ...
    for (let i = 0, j = m - 1; i < m; j = i++) {
        a2 += (p2[j][0] + p2[i][0]) * (p2[j][1] - p2[i][1]);
    }
 ...
 */
function tessellateThinRing(surface, loops, sampled, fid, verts, faceIds, sign, tol) {
    ...
    if (loops.length !== 2 || surface.kind !== "PLANE")
        return false;
    const rail = (lp) => {
        const p = [];
        for (const oe of lp.edges) {
    ...
    if (!r1 || !r2raw) {
        if (DBG)
            console.error(`[tess] fid=${fid} thinRing bail: rail missing (${r1 ? r1.length : "null"}/${r2raw ? r2raw.length : "null"})`);
        return false;
    }
    const d3 = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
    ...
    const perim = (r) => { let s = 0; for (let i = 0; i < r.length; i++)
        s += d3(r[i], r[(i + 1) % r.length]); return s; };
    const area = (r) => {
        let a = 0;
    ...
    if (width > tol) {
        if (DBG)
            console.error(`[tess] fid=${fid} thinRing bail: width ${width.toExponential(2)} > ${tol}`);
        return false;
    }
    ...
    for (let i = 0; i < r1.length; i += st1) {
        if (distToPolyline(r1[i], closed2) > near) {
            if (DBG)
                console.error(`[tess] fid=${fid} thinRing bail: rims not nested (r1[${i}] > ${near.toExponential(2)})`);
            return false;
        }
    }
    for (let j = 0; j < r2raw.length; j += st2) {
        if (distToPolyline(r2raw[j], closed1) > near) {
            if (DBG)
                console.error(`[tess] fid=${fid} thinRing bail: rims not nested (r2[${j}] > ${near.toExponential(2)})`);
            return false;
        }
    }
    ...
    const frac = (c) => {
        ...
        for (let i = 1; i <= c.length; i++)
            f.push(f[i - 1] + d3(c[i - 1], c[i % c.length]));
 ...
 */
function tessellateRibbon(surface, loops, sampled, fid, verts, faceIds, sign) {
    const d3 = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
    const loopEntries = [];
    for (const lp of loops) {
        const es = [];
    ...
    const loftOpen = (c1, c2in) => {
        let c2 = c2in;
        if (d3(c1[0], c2[0]) > d3(c1[0], c2[c2.length - 1]))
            c2 = c2.slice().reverse();
        const na = c1.length, nb = c2.length;
        if (na < 2 || nb < 2)
            return false;
    ...
    const loftClosed = (c1in, c2in) => {
        ...
        const track = (cc) => {
            ...
            for (let k = 0; k < 16; k++) {
                const t = k / 16;
                s += d3(at(c1, f1, t), at(cc, f2, t));
            }
            return s;
        };
        const rev = [c2[0], ...c2.slice(1).reverse()];
        if (track(rev) < track(c2))
            c2 = rev;
        ...
        let i = 0, j = 0;
        while (i < na || j < nb) {
            if (i < na && (j >= nb || f1[i + 1] <= f2[j + 1])) {
                tris.push([c1[i % na], c1[(i + 1) % na], c2[j % nb]]);
                i++;
            }
            else {
                tris.push([c1[i % na], c2[(j + 1) % nb], c2[j % nb]]);
                j++;
            }
        }
        let vote = 0;
    ...
    const railsA = () => {
        ...
        for (let i = 0; i < flat.length; i++) {
            ...
            for (let j = i + 1; j < flat.length; j++) {
                ...
                if (reversed && d3(mid(pi), mid(pj)) < 1e-6) {
                    used[j] = true;
                    break;
                }
            }
    ...
    const railsB = () => {
        ...
        for (const es of loopEntries) {
            ...
            if (![...count.values()].some((c) => c === 2))
                return null; // no slit in this loop
            const keep = es.map((e) => count.get(e.id) !== 2);
            if (!keep.some((k) => k))
                return null; // nothing but slits
            const n = es.length;
            let anchor = 0;
            while (anchor < n && keep[anchor])
                anchor++; // a dropped slot; exists since some edge doubled
            let run = null;
            for (let s = 1; s <= n; s++) {
                const i = (anchor + s) % n;
                if (!keep[i]) {
                ...
                if (!run)
                    ...
                    run.push(...(d3(run[run.length - 1], poly[0]) < 1e-6 ? poly.slice(1) : poly));
            }
            if (run && run.length >= 2)
                chains.push(run);
        }
 ...
 */
function tessellateConeCap(surface, outerLoop, sampled, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign) {
    const cone = surface;
    if (surface.kind !== "CONICAL_SURFACE" || !(cone.sin > 0))
        return false;
    const rim = [];
    for (const oe of outerLoop.edges) {
        const base = sampled.get(oe.edgeId);
    ...
    if (Math.abs(Math.abs(travel) - TWO_PI) > 0.1 * TWO_PI)
        return false;
    ...
    for (let j = 1; j <= nRings; j++) {
        if (j === nRings) {
            stitchRings(verts, faceIds, prev, [apex], fid, surface, sign);
            break;
        }
        const t = 1 - j / nRings;
        const ring = rim.map((p) => [apex[0] + t * (p[0] - apex[0]), apex[1] + t * (p[1] - apex[1]), apex[2] + t * (p[2] - apex[2])]);
        stitchRings(verts, faceIds, prev, ring, fid, surface, sign);
        prev = ring;
    }
...
function tessellateCone(surface, loop, sampled, brep, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign, nFaceLoops) {
    ...
    const circleEdges = loop.edges.filter((oe) => {
        ...
        if (!closed)
            return false;
        if (brep.table.typeOf(e.curveId) === "CIRCLE")
            return true;
    ...
    if (circleEdges.length === 1) {
        ...
        for (const oe of loop.edges) {
            ...
            for (const p of [e.v0, e.v1])
                slant = Math.max(slant, Math.abs(surface.project(p)[1] - vApexP));
        }
        const atApex = (p) => Math.abs(surface.project(p)[1] - vApexP) < 0.02 * Math.max(slant, 1e-9);
        const chain = [];
        for (const oe of loop.edges) {
            const e = brep.edges.get(oe.edgeId);
            ...
            if (!s || !e)
                return false;
            if (atApex(e.v0) || atApex(e.v1))
                continue; // seam ruling to the apex — not part of the rim
        ...
        if (vmax - vmin > 1e-3 * Math.max(slantV, 1e-9))
            return false;
        if (Math.abs(cycleWind(uv, 0, TWO_PI)) !== 1)
            return false;
        base = chain;
        multiEdgeRim = true;
    }
    ...
    for (const oe of loop.edges) {
        ...
        for (const v of [e.v0, e.v1])
            minApex = Math.min(minApex, Math.hypot(v[0] - apex[0], v[1] - apex[1], v[2] - apex[2]));
    }
    ...
    for (let j = 1; j <= nV; j++) {
        ...
        const ring = [];
        for (let k = 0; k < M; k++)
            ring.push(surface.evaluate(theta0 + (dir * TWO_PI * k) / M, vf));
        stitchRings(verts, faceIds, prev, ring, fid, surface, sign);
        prev = ring;
    }
    return true;
 ...
 */
function tessellateConeSlice(surface, loop, sampled, brep, fid, verts, faceIds, sign) {
    const cone = surface;
    const apex = surface.evaluate(0, -cone.r / cone.sin);
    const d3 = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
        ...
        if (isSeam) {
            ...
            if (len < 1e-9)
                ...
                for (const p of s) {
                    ...
                    if (Math.hypot(px - t * dx, py - t * dy, pz - t * dz) > 1e-5 * len + 1e-9) {
                        isSeam = false;
                        break;
                    }
                }
    ...
    const bail = (why) => { if (DBG && process.env.MESHSTEP_SLICEDBG)
        console.error(`[coneSlice] fid=${fid} bail: ${why}`); return false; };
    if (arcs.length !== 1 || seams.length !== 2)
        return bail(`arcs=${arcs.length} seams=${seams.length}`);
    const arcEdge = brep.edges.get(arcs[0]);
    if (d3(arcEdge.v0, arcEdge.v1) < 1e-9)
        return bail("closed rim"); // full circle -> tessellateCone / band
    ...
    if (!Lc || !Rc || Lc.length !== Rc.length)
        return bail(`seams: L=${Lc?.length ?? "far-from-apex"} R=${Rc?.length ?? "far-from-apex"} apexTol=${apexTol.toExponential(1)}`); // asymmetric sampling -> param grid
    const n = Lc.length;
    ...
    if (n * (sampled.get(arcs[0])?.length ?? 0) > 2_000_000)
        return false;
    ...
    if (!typed && n * (sampled.get(arcs[0])?.length ?? 0) > 50_000)
        return bail(`geometric wedge too large (${n}×${sampled.get(arcs[0])?.length}) — graded grid handles it`);
    ...
    if (d3(arc[0], Lrim) > d3(arc[arc.length - 1], Lrim))
        arc = arc.reverse();
    if (d3(arc[0], Lrim) > d3(arc[0], Rrim)) {
        const t = Lc;
        Lc = Rc;
        Rc = t;
    }
    ...
    const angs = arc.map((p) => surface.project(p)[0]);
    for (let i = 1; i < M; i++) {
    ...
    const ringAt = (k) => {
        ...
        const r = [];
        for (let j = 0; j < M; j++)
            r.push(surface.evaluate(angs[j], v));
        return r;
    };
    if (DBG && n * M > 100000)
        console.error(`[coneSlice] fid=${fid} n=${n} M=${M} -> ~${n * M * 2} tris`);
    // OPEN stitch (the wedge is not a full revolution, so rings must NOT wrap left-to-right).
    const openStitch = (A, B) => {
        if (A.length === 1) {
            for (let j = 0; j + 1 < B.length; j++)
                emitTri(verts, faceIds, A[0], B[j], B[j + 1], fid, surface, sign);
            return;
        }
        if (B.length === 1) {
            for (let j = 0; j + 1 < A.length; j++)
                emitTri(verts, faceIds, A[j], A[j + 1], B[0], fid, surface, sign);
            return;
        }
 ...
 */
function tessellateFullPeriodic(surface, fid, chordTol, targetEdge, normalDev, sign, verts, faceIds, brep, sampled) {
    if (!surface.periodicU)
        return false;
    const openV = !surface.periodicV;
    const vDom = surface.vDomain;
    if (openV && (!vDom || !(vDom[1] - vDom[0] > 1e-12)))
        return false;
    ...
    const borrowRim = (v) => {
        ...
        const probe = [];
        for (let i = 0; i < 8; i++)
            probe.push(surface.evaluate(u0 + (uP * i) / 8, v));
        let cx = 0, cy = 0, cz = 0;
        ...
        if (nl < 1e-18 || r < 1e-9)
            return null; // degenerate rim (pole) — nothing to borrow
        nx /= nl;
        ny /= nl;
        nz /= nl;
        const off = (q) => {
        ...
        for (const [id, e] of brep.edges) {
            if (off(e.v0) > eps || off(e.v1) > eps)
                continue;
            const poly = sampled.get(id);
            if (!poly || poly.length < 4)
                continue;
            if (dd(poly[0], poly[poly.length - 1]) > eps)
                continue; // want a full closed ring
            let onRing = true;
            for (const q of poly)
                if (off(q) > eps) {
                    onRing = false;
    ...
    const alignRing = (ref, ring) => {
        let bi = 0, bd = Infinity;
        for (let i = 0; i < ring.length; i++) {
            const d = dd(ref[0], ring[i]);
            if (d < bd) {
                bd = d;
                bi = i;
            }
        ...
        const at = (c, f) => c[Math.min(c.length - 1, Math.round(f * c.length)) % c.length];
        if (dd(q, at(rev, 0.25)) < dd(q, at(out, 0.25)))
            out = rev;
        return out;
    };
    ...
    const ringAt = (v) => {
        ...
        for (let i = 1; i <= K; i++) {
            const q = surface.evaluate(u0 + (uP * i) / K, v);
            circ += dd(prev, q);
            prev = q;
        }
        ...
        const r = [];
        for (let i = 0; i < nu; i++)
            r.push(surface.evaluate(u0 + (uP * i) / nu, v));
        return r;
    };
    const start = verts.length;
    const rimAt = (v) => {
        ...
        const b = borrowRim(v);
        if (DBG && b)
            console.error(`[tess] fid=${fid} fullPeriodic: borrowed ${b.length}-pt rim at v=${v}`);
 ...
 */
function tessellateCapDome(surface, loop, sampled, fid, verts, faceIds, chordTol, targetEdge, normalDev, sign) {
    ...
    const evalPS = (p, s) => (P === 0 ? surface.evaluate(p, s) : surface.evaluate(s, p));
    // Rim = the loop's once-used edges chained (a doubled edge is a degenerate pole seam, excluded).
    const count = new Map();
    for (const oe of loop.edges)
        count.set(oe.edgeId, (count.get(oe.edgeId) ?? 0) + 1);
    const rim = [];
    for (const oe of loop.edges) {
        if ((count.get(oe.edgeId) ?? 0) !== 1)
            continue;
        const base = sampled.get(oe.edgeId);
        if (!base)
            return false;
    ...
    let hu, hv;
    for (let i = 0; i < rim.length; i += stride) {
        const q = surface.project(rim[i], hu, hv);
        uv.push(q);
        hu = q[0];
        hv = q[1];
    }
    ...
    if (Math.abs(Math.abs(travel) - period) > 0.1 * period)
        return false;
    ...
    const ringSpread = (s) => {
        ...
        const c = evalPS(0, s);
        for (let i = 1; i <= 8; i++)
            mx = Math.max(mx, dd(c, evalPS((period * i) / 8, s)));
        return mx;
    };
    ...
    let sArc = 0;
    {
        ...
        for (let i = 1; i <= 16; i++) {
            const q = evalPS(p0, sRim + ((sPole - sRim) * i) / 16);
            sArc += dd(prev, q);
            prev = q;
        }
    }
    ...
    for (let j = 1; j <= nV; j++) {
        ...
        if (j === nV) {
            stitchRings(verts, faceIds, prev, [evalPS(pStart, sPole)], fid, surface, sign);
            break;
        }
        let circ = 0;
        {
            ...
            for (let i = 1; i <= 16; i++) {
                const q = evalPS(pStart + pDir * (period * i) / 16, s);
                circ += dd(pv, q);
                pv = q;
            }
        }
        ...
        const ring = [];
        for (let i = 0; i < nu; i++)
            ring.push(evalPS(pStart + pDir * (period * i) / nu, s));
        stitchRings(verts, faceIds, prev, ring, fid, surface, sign);
        prev = ring;
    }
    return verts.length > start;
 ...
 */
function tessellatePeriodicRegion(surface, loops, sampled, fid, verts, faceIds, targetEdge, chordTol, normalDev, sign) {
    const uPer = !!surface.periodicU, vPer = !!surface.periodicV;
    if ((!uPer && !vPer) || loops.length === 0)
        return false;
    ...
    for (const l of lifted) {
        if (l.p2.length < 3 || l.p2.length !== l.p3.length) {
            if (DBG)
                console.error(`[tess] fid=${fid} region bail: lift p2=${l.p2.length} p3=${l.p3.length}`);
            return false;
        }
    }
    let P;
    if (uPer && !vPer)
        ...
        if (wU && wV) {
            if (DBG)
                console.error(`[tess] fid=${fid} region bail: loops wind both directions`);
            return false;
        }
        P = wV ? 1 : 0;
    }
    ...
    const sDom = sPeriodic ? [sSeam, sSeam + sPeriod]
    ...
    for (const l of lifted) {
        ...
        if (wS !== 0) {
            if (DBG)
                console.error(`[tess] fid=${fid} region bail: loop winds S (${l.windU}/${l.windV})`);
            return false;
        }
        let mean = 0;
        for (const q of l.p2)
        ...
        if (wP === 1 || wP === -1)
            ...
            if (DBG)
                console.error(`[tess] fid=${fid} region bail: multi-wind ${wP}`);
            return false;
        }
    }
    if (rims.length > 2) {
        if (DBG)
            console.error(`[tess] fid=${fid} region bail: ${rims.length} rims`);
        return false;
    }
    if (!sPeriodic && rims.length === 2 && rims[0].wind === rims[1].wind)
        return false;
    if (rims.length < 2 && !sDom)
        return false; // open stack (cylinder/cone) needs rims for S-bounds
    ...
    const candidates = [];
    if (spans.length === 0)
        candidates.push(p0 + period / 2, p0 + period / 4, p0);
    else {
        // Circular gaps between hole spans (spans wider than the period leave no corridor).
        const marks = spans.map(([mn, mx]) => [norm(mn), norm(mn) + Math.min(mx - mn, period)])
            .sort((a, b) => a[0] - b[0]);
        for (let i = 0; i < marks.length; i++) {
            const end = marks[i][1];
            const nextStart = i + 1 < marks.length ? marks[i + 1][0] : marks[0][0] + period;
            if (nextStart - end > 1e-6 * period)
                candidates.push(norm((end + nextStart) / 2));
        }
        candidates.sort((a, b) => {
            const gap = (c) => {
                let g = Infinity;
    ...
    const syntheticChain = (sEnd, uStar, asc) => {
        ...
        for (let i = 0; i <= nP; i++) {
            ...
            q[S] = sEnd;
            p2.push(q);
            p3.push(evalPS(u, sEnd));
        }
        return { p2, p3 };
    };
    const ascending = (c) => c.p2[c.p2.length - 1][P] >= c.p2[0][P] ? c : { p2: c.p2.slice().reverse(), p3: c.p3.slice().reverse() };
    const descending = (c) => c.p2[c.p2.length - 1][P] <= c.p2[0][P] ? c : { p2: c.p2.slice().reverse(), p3: c.p3.slice().reverse() };
    const shiftS = (l, by) => ({
        p2: l.p2.map((q) => { const r = [q[0], q[1]]; r[S] = r[S] + by; return r; }),
        p3: l.p3, wind: l.wind, meanS: l.meanS + by,
    });
    let sLowEnd = null, sHighEnd = null; // synthetic ends (else rim)
    let rimLow = null, rimHigh = null;
    ...
    const anchor = (c, uStar, byEnd) => {
        const refU = byEnd ? c.p2[c.p2.length - 1][P] : c.p2[0][P];
        const k = Math.round((uStar - refU) / period);
        if (k === 0)
            return c;
        return { p2: c.p2.map((q) => { const r = [q[0], q[1]]; r[P] = r[P] + k * period; return r; }), p3: c.p3 };
    };
    ...
    for (const uStar of candidates.slice(0, 4)) {
        ...
        if (Math.abs(A[P] - B[P]) > 0.3 * period) {
            if (DBG)
                console.error(`[tess] fid=${fid} region: cut drift ${(A[P] - B[P]).toFixed(3)} at u*=${uStar.toFixed(3)}`);
            continue;
        }
        ...
        for (let i = 1; i < nSeam; i++) {
            ...
            q[S] = A[S] + (B[S] - A[S]) * t;
            seamR.p2.push(q);
            seamR.p3.push(evalPS(q[P], q[S]));
        }
        const seamL = { p2: [], p3: [] };
        for (let i = seamR.p2.length - 1; i >= 0; i--) {
            const q = [seamR.p2[i][0], seamR.p2[i][1]];
            q[P] = q[P] - period;
            seamL.p2.push(q);
            seamL.p3.push(seamR.p3[i]); // identical 3D point — the weld seals the seam
        }
        // Assemble the outer polygon: low (asc) + right seam + high (desc) + left seam.
        const outerP2 = [...low.p2, ...seamR.p2, ...high.p2, ...seamL.p2];
        const outerP3 = [...low.p3, ...seamR.p3, ...high.p3, ...seamL.p3];
        if (outerP2.length > 40000) {
            if (DBG)
                console.error(`[tess] fid=${fid} region: band boundary too dense (${outerP2.length})`);
            return false;
        }
        if (outerP2.length < 3 || countSelfIntersections(outerP2, 1) > 0) {
            if (DBG)
                console.error(`[tess] fid=${fid} region: band polygon self-intersects at u*=${uStar.toFixed(3)}`);
            continue;
        }
        ...
        let holesOk = true;
        for (const h of holes) {
            let hp = h.p2.map((q) => [q[0], q[1]]);
            let cen = [0, 0];
            const centroid = () => { cen = [0, 0]; for (const q of hp) {
                cen[0] += q[0];
                cen[1] += q[1];
            } cen[0] /= hp.length; cen[1] /= hp.length; };
            centroid();
            const lo = Math.min(outerP2[0][P], A[P] - period);
            let shift = 0;
            while (cen[P] + shift < lo)
                shift += period;
            while (cen[P] + shift > lo + period)
                shift -= period;
            hp = hp.map((q) => { const r = [q[0], q[1]]; r[P] = r[P] + shift; return r; });
            centroid();
            let inside = pointInPoly(cen, outerP2);
            // Wrong period branch: retry ±period in P, and on a doubly-periodic surface ±period in S too.
            const alts = [[period, 0], [-period, 0]];
            if (sPeriodic)
                alts.push([0, sPeriod], [0, -sPeriod], [period, sPeriod], [period, -sPeriod], [-period, sPeriod], [-period, -sPeriod]);
            for (const [aP, aS] of alts) {
                if (inside)
                    break;
                const test = hp.map((q) => { const r = [q[0], q[1]]; r[P] = r[P] + aP; r[S] = r[S] + aS; return r; });
                let c2 = [0, 0];
                for (const q of test) {
                    c2[0] += q[0];
                    c2[1] += q[1];
        ...
        if (!holesOk) {
            if (DBG)
                console.error(`[tess] fid=${fid} region: hole outside band at u*=${uStar.toFixed(3)}`);
            continue;
        }
        // Interior grid over the band's bounding box, kept clear of every boundary segment.
        let sMin = Infinity, sMax = -Infinity;
        for (const q of outerP2) {
            if (q[S] < sMin)
                sMin = q[S];
            if (q[S] > sMax)
                sMax = q[S];
        ...
        if (nU * nV > capCells) {
            const f = Math.sqrt(capCells / (nU * nV));
            nU = Math.max(4, Math.floor(nU * f));
            nV = Math.max(2, Math.floor(nV * f));
        }
        const uLo = A[P] - period;
        const du = period / nU, dv = (sMax - sMin) / nV;
        // Bucket boundary segments by u-column for the keep-out test.
        const segs = [];
        const pushSegs = (poly, closed) => {
        ...
        for (const hp of holeP2)
            pushSegs(hp, true);
        const bins = Array.from({ length: nU + 2 }, () => []);
        ...
        const clearOf = (q, col) => {
            for (const si of bins[Math.max(0, Math.min(nU + 1, col))]) {
                ...
                if (dx * dx + dy * dy < 0.36 * cellM * cellM)
                    return false;
            }
            return true;
        };
        ...
        for (let hi = 0; hi < holeP2.length; hi++) {
            ...
            for (let i = 0; i < holeP2[hi].length; i++) {
                pts2.push(holeP2[hi][i]);
                pts3.push(holeP3[hi][i]);
            }
        ...
        for (let i = 0; i < nU; i++) {
            ...
            for (let j = 0; j < nV; j++) {
                const s = sMin + (j + 0.5) * dv;
                while (k < xs.length && xs[k] < s)
                    k++;
                if (k % 2 === 0)
                    continue; // even crossings below → outside
                if (i % rowStride[j] !== 0)
                    continue; // thinned: this row can't support full u-density
                const q = [0, 0];
                q[P] = u;
                q[S] = s;
                if (!clearOf(q, i + 1))
        ...
        const scaled = pts2.map((q) => [q[P] * mP, q[S] * mS]);
        const out = { missing: 0 };
        const tris = constrainedTriangulate(scaled, [outerIdx, ...holeIdx], interiorIdx, out);
        if (out.missing > 0 || tris.length === 0) {
            if (DBG)
                console.error(`[tess] fid=${fid} region: CDT missing=${out.missing} tris=${tris.length} at u*=${uStar.toFixed(3)} — retry/bail`);
            if (++cdtFails >= 2)
                return false;
            continue;
        }
        ...
        if (DBG)
            console.error(`[tess] fid=${fid} region: rims=${rims.length} holes=${holes.length} tris=${tris.length} nU=${nU} nV=${nV}`);
        if (verts.length > start)
            return true;
    }
    return false;
}
function tessellateSphere(s, fid, chordTol, targetEdge, normalDev, sign, verts, faceIds) {
    ...
    const ringAt = (v) => {
        ...
        if (nu <= 2)
            return [s.evaluate(0, v)]; // pole
        const r = [];
        for (let i = 0; i < nu; i++)
            r.push(s.evaluate((TWO_PI * i) / nu, v));
        return r;
    };
 ...
 */
function tessellateSphereCap(s, loop, sampled, fid, verts, faceIds, chordTol, targetEdge, normalDev, sign) {
    ...
    if (vmax - vmin > 0.05 || umax - umin < 0.9 * TWO_PI)
        return false;
    ...
    let du = 0;
    for (let i = 0; i < uv.length; i++) {
        let d = uv[(i + 1) % uv.length][0] - uv[i][0];
        while (d > Math.PI)
            d -= TWO_PI;
        while (d < -Math.PI)
            d += TWO_PI;
    ...
    for (let j = 1; j <= nV; j++) {
        ...
        if (j === nV) {
            stitchRings(verts, faceIds, prev, [s.evaluate(0, vPole)], fid, s, sign);
            break;
        }
        const circ = TWO_PI * R * Math.cos(v);
        ...
        const ring = [];
        for (let i = 0; i < nu; i++)
            ring.push(s.evaluate(uStart + uDir * (TWO_PI * i) / nu, v));
        stitchRings(verts, faceIds, prev, ring, fid, s, sign);
        prev = ring;
    }
    return true;
 ...
 */
function tessellateBSplineFull(s, fid, chordTol, targetEdge, normalDev, sign, verts, faceIds) {
    const { u0, u1, v0, v1 } = s;
    const arcLen = (along) => {
        let len = 0;
        const M = 64;
    ...
    for (let i = 0; i <= nU; i++) {
        ...
        let circ = 0;
        let prev = s.evaluate(u, v0);
        const SAMP = 48;
        for (let j = 1; j <= SAMP; j++) {
            const p = s.evaluate(u, v0 + ((v1 - v0) * j) / SAMP);
            circ += Math.hypot(p[0] - prev[0], p[1] - prev[1], p[2] - prev[2]);
            prev = p;
        }
        const nv = Math.max(1, Math.min(4000, Math.round(circ / target)));
        const ring = [];
        if (nv <= 2)
            ring.push(s.evaluate(u, (v0 + v1) / 2)); // collapsed ring = pole point
        else
            for (let j = 0; j < nv; j++)
                ring.push(s.evaluate(u, v0 + ((v1 - v0) * j) / nv)); // closed (wraps via modulo)
        rings.push(ring);
    }
    // Stitch consecutive rings (handles unequal counts and pole fans); emitTri fixes winding.
    for (let i = 0; i < nU; i++)
        stitchRings(verts, faceIds, rings[i], rings[i + 1], fid, s, sign);
    return true;
}
/**
...
function readTessellated(brep) {
    ...
    if (containers.length === 0)
        return null;
    const verts = [], faceIds = [], solidIds = [];
    let faces = 0;
    const coordsCache = new Map();
    const getCoords = (id) => {
        let c = coordsCache.get(id);
        if (!c) {
            c = list(t.record(id).params[2]).map((tup) => { const a = numList(tup); return [a[0] * s, a[1] * s, a[2] * s]; });
    ...
    for (const [cid, c] of containers) {
        for (const fref of refList(c.params[1])) {
            ...
            if (rec.type !== "TRIANGULATED_FACE" && rec.type !== "COMPLEX_TRIANGULATED_FACE")
                continue;
            ...
            for (let i = 3; i < params.length; i++) {
                const p = params[i];
                if (p.k === "list" && p.v.length === pnmax && (p.v.length === 0 || p.v[0].k === "num")) {
                    pidx = i;
                    break;
                }
            }
            ...
            const vtx = (local) => pts[pnindex[local - 1] - 1];
            const pushTri = (a, b, cc) => {
                if (a === b || b === cc || a === cc)
                    return; // strip-restart degenerate (repeated index) — skip
                const A = vtx(a), B = vtx(b), C = vtx(cc);
                verts.push(A[0], A[1], A[2], B[0], B[1], B[2], C[0], C[1], C[2]);
                faceIds.push(fref);
                solidIds.push(cid);
            };
            const addList = (p, kind) => {
                for (const sub of list(p)) {
                    ...
                    if (kind === "tri")
                        ...
                        for (let i = 0; i + 2 < idx.length; i++)
                            (i % 2 === 0) ? pushTri(idx[i], idx[i + 1], idx[i + 2]) : pushTri(idx[i + 1], idx[i], idx[i + 2]);
                    else
                        for (let i = 1; i + 1 < idx.length; i++)
                            pushTri(idx[0], idx[i], idx[i + 1]);
                }
            };
            const rest = params.slice(pidx + 1);
            if (rec.type === "TRIANGULATED_FACE") {
                if (rest[0])
                    addList(rest[0], "tri");
            }
            else if (rest.length >= 3) {
                addList(rest[0], "tri");
                addList(rest[1], "strip");
                addList(rest[2], "fan");
            }
            else {
                if (rest[0])
                    addList(rest[0], "strip");
                if (rest[1])
                    addList(rest[1], "fan");
            }
            faces++;
...
export function tessellate(brep, opts = {}) {
    ...
    const mark = (fid, n, r) => {
        if (r) {
            trace?.(fid, n);
            if (DBG)
                console.error(`[dispatch] fid=${fid} -> ${n}`);
        }
        return r;
    };
    const skipped = {};
    ...
    beginWarnings();
    for (const fid of brep.droppedFaces)
        warn("face-dropped", fid, "malformed face record dropped while reading the B-rep — geometry missing");
    // Cache each face's surface (used to dispatch tessellation).
    const faceSurf = new Map();
    ...
    for (const solid of brep.solids)
        pTotal += (W_FACE + W_STITCH) * solid.faces.length;
    const tick = (units) => { pDone += units; opts.onProgress?.(pDone, pTotal); };
    ...
    {
        const level = new Map(); // edgeId -> finest committed refinement factor
        const resample = (id, f) => {
            ...
            for (const face of solid.faces) {
                ...
                const tangledLoops = face.loops.filter((lp) => {
                    ...
                    if (p2.length < 4)
                        return false;
                    const pairs = [];
                    if (countSelfIntersections(p2, 64, pairs) === 0)
                        return false;
                    if (!small) {
                        const n = p2.length;
                        for (const [i, j] of pairs) {
                            if (Math.min((j - i + n) % n, (i - j + n) % n) > 8) {
                                nonLocal = true;
                                break;
                            }
                        }
                ...
                if (tangledLoops.length === 0 || nonLocal)
                    continue;
                for (const f of [4, 16, 64]) {
                    const probe = new Map(sampled);
                    const ids = new Set();
                    for (const lp of tangledLoops)
                        for (const oe of lp.edges)
                            ids.add(oe.edgeId);
                    for (const id of ids)
                        probe.set(id, resample(id, f));
                    ...
                    if (!stillBad) {
                        if (DBG)
                            console.error(`[tess] micro-face densify: fid=${face.faceId} simple at tol/${f} (${ids.size} edges)`);
                        for (const id of ids)
                            level.set(id, Math.max(level.get(id) ?? 0, f));
                        break;
                    }
                    if (f === 64) {
                        ...
                        if (DBG)
                            console.error(`[tess] micro-face densify: fid=${face.faceId} still self-intersecting at tol/64 — left as-is (genuine overlap)`);
                    }
                }
            }
        for (const [id, f] of level)
            sampled.set(id, resample(id, f));
    }
    ...
    for (const solid of brep.solids) {
        ...
        for (const face of solid.faces) {
            ...
            if (ok)
                ...
                if (DBG)
                    console.error(`[tess] UNTRIANGULATED fid=${face.faceId} kind=${face.surfaceKind} loops=${face.loops.map((l) => l.edges.length).join("/")}`);
        ...
        tick(stitchQ);
        const fill = fillMicroHoles(z.positions, tj.indices, tj.faceOf, 0.05, solid.open ? undefined : { edge: targetEdge, dev: 0.75 * chordTol });
        for (const x of z.positions)
            positions.push(x);
        for (const ix of tj.indices)
            indices.push(ix + voff);
        for (const ix of fill.indices)
            indices.push(ix + voff);
        voff += z.positions.length / 3;
        for (const f of tj.faceOf) {
            faceOfTri.push(f);
            solidOfTri.push(solid.id);
        }
        for (const f of fill.faceOf) {
            faceOfTri.push(f);
            solidOfTri.push(solid.id);
        }
        tick(stitchQ);
    ...
    if (brep.solids.length > 1) {
        ...
        const unsewn = new Set();
        const sewnSolids = new Set();
        const sewn = sewSolids(positions, indices, faceOfTri, solidOfTri, Math.max(0.02, 2 * chordTol), openIds, unsewn, sewnSolids);
        if (sewn > 0)
            warn("heuristic-fill", -1, `${sewn} coincident open rim pair(s) sewn across solids`);
        ...
        for (const solid of brep.solids) {
            if (solid.open || !unsewn.has(solid.id))
                continue;
            ...
            let boundary = 0;
            for (const n of useCount.values())
                if (n === 1)
                    boundary++;
            if (boundary > 0) {
                solid.open = true;
                ...
                if (DBG)
                    console.error(`[tess] sewSolids: solid ${solid.id} reclassified open (${boundary} B-rep boundary edges, unsewn rings)`);
            }
        }
    }
    ...
    if (brep.solids.length === 0) {
        ...
        if (tg && tg.verts.length > 0) {
            ...
            for (const x of mesh.positions)
                positions.push(x);
            for (const ix of mesh.indices)
                indices.push(ix + voff);
            voff += mesh.positions.length / 3;
            for (let k = 0; k < tg.faceIds.length; k++) {
                faceOfTri.push(tg.faceIds[k]);
                solidOfTri.push(tg.solidIds[k]);
            }
            facesTotal += tg.faces;
            facesTessellated += tg.faces;
        }
    }
    return {
 ...
 */
function zipSlivers(mesh, tol) {
    const I = mesh.indices, P = mesh.positions, nv = P.length / 3, nt = I.length / 3;
    const keepAll = () => { const k = new Array(nt); for (let t = 0; t < nt; t++)
        k[t] = true; return k; };
    const KEY = 2 ** 26;
    ...
    for (let i = 0; i < I.length; i += 3)
        for (let e = 0; e < 3; e++) {
            ...
            if (use.get(ek(a, b)) === 1) {
                ...
                vCap.set(a, Math.min(vCap.get(a) ?? Infinity, L));
                vCap.set(b, Math.min(vCap.get(b) ?? Infinity, L));
            }
        }
    if (!openV.size)
        return { positions: P, indices: I, keep: keepAll() };
    ...
    const uni = (a, b) => { const ra = find(a), rb = find(b); if (ra !== rb)
        parent[Math.max(ra, rb)] = Math.min(ra, rb); };
    const cell = Math.max(tol, 1e-9);
    const px = (i) => P[i * 3], py = (i) => P[i * 3 + 1], pz = (i) => P[i * 3 + 2];
    const ckey = (x, y, z) => `${Math.round(x / cell)},${Math.round(y / cell)},${Math.round(z / cell)}`;
    const hash = new Map();
    for (const v of openV) {
        const k = ckey(px(v), py(v), pz(v));
            ...
            for (let gy = -1; gy <= 1; gy++)
                for (let gz = -1; gz <= 1; gz++) {
                    for (const w of hash.get(`${cx + gx},${cy + gy},${cz + gz}`) ?? []) {
                        if (w === v || find(w) === find(v) || use.has(ek(v, w)))
                            continue;
    ...
    const pos = [];
    const idxOf = (v) => { const r = find(v); if (remap[r] === -1) {
        remap[r] = pos.length / 3;
        pos.push(px(r), py(r), pz(r));
    } return remap[r]; };
    const outI = [];
    const keep = new Array(nt);
    for (let t = 0; t < nt; t++) {
 ...
 */
function zipTJunctions(P, I0, faceOf0, tol) {
    ...
    for (let round = 0; round < 3; round++) {
        ...
        for (let i = 0; i < I.length; i += 3)
            for (let e = 0; e < 3; e++)
                use.set(ek(I[i + e], I[i + (e + 1) % 3]), (use.get(ek(I[i + e], I[i + (e + 1) % 3])) ?? 0) + 1);
        const openE = [];
        const openV = new Set();
        for (let i = 0; i < I.length; i += 3)
            for (let e = 0; e < 3; e++) {
        ...
        const ins = new Map(); // key tri*3+edge
        let found = 0;
        for (const oe of openE) {
            ...
            for (let si = 0; si <= steps; si++) {
                ...
                for (let gx = -1; gx <= 1; gx++) {
                    ...
                    for (let gy = -1; gy <= 1; gy++)
                        for (let gz = -1; gz <= 1; gz++) {
                            if (Math.abs(cx + gx - pcx) <= 1 && Math.abs(cy + gy - pcy) <= 1 && Math.abs(cz + gz - pcz) <= 1)
                                continue; // covered last step
                            ...
                            for (const v of arr) {
                                ...
                                if (t * len < tol || (1 - t) * len < tol)
                                    continue; // too near an endpoint — weld territory
                                const d = Math.hypot(qx - t * ex, qy - t * ey, qz - t * ez);
                                if (d <= tol && !list.some((x) => x.v === v))
                                    list.push({ v, t });
                            }
                        }
                }
            ...
            if (list.length) {
                ins.set(oe.tri * 3 + oe.edge, list.sort((x, y) => x.t - y.t));
                found += list.length;
            }
        }
        if (!found)
        ...
        for (let t = 0; t < I.length / 3; t++) {
            ...
            if (!anyIns) {
                outI.push(I[t * 3], I[t * 3 + 1], I[t * 3 + 2]);
                outF.push(faceOf[t]);
                continue;
            }
            ...
            for (let e = 0; e < 3; e++) {
                poly.push(I[t * 3 + e]);
                for (const x of ins.get(t * 3 + e) ?? [])
                    poly.push(x.v);
            }
            for (let i = 1; i + 1 < poly.length; i++) {
                outI.push(poly[0], poly[i], poly[i + 1]);
                outF.push(faceOf[t]);
            }
        }
 ...
 */
function sewSolids(P, I, faceOfTri, solidOfTri, tol, openIds, unsewn, sewnSolids) {
    const KEY = 2 ** 26;
    const ek = (a, b) => (a < b ? a * KEY + b : b * KEY + a);
    const use = new Map();
    for (let i = 0; i < I.length; i += 3)
    ...
    for (const start of nxt.keys()) {
        ...
        let r = 0;
        for (const a of v)
            r = Math.max(r, Math.hypot(P[a * 3] - cx, P[a * 3 + 1] - cy, P[a * 3 + 2] - cz));
        const frac = [0];
    ...
    if (DBG)
        console.error(`[tess] sewSolids: ${rings.length} open rings: ${rings.slice(0, 12).map((r) => `s${r.solid}×${r.v.length}(per=${r.per.toFixed(2)})`).join(" ")}`);
    if (rings.length < 2) {
    ...
    for (let i = 0; i < rings.length; i++) {
        ...
        for (let j = 0; j < rings.length; j++) {
            ...
            if (openIds.has(A.solid) !== openIds.has(B.solid))
                continue;
            if (Math.hypot(B.cx - A.cx, B.cy - A.cy, B.cz - A.cz) > 0.5 * (A.r + B.r) + tol)
                continue;
            if (Math.abs(A.per - B.per) > 0.2 * Math.max(A.per, B.per) + 4 * tol)
                continue;
        ...
        paired.add(i);
        paired.add(bj);
        sewnSolids?.add(A.solid);
        sewnSolids?.add(B.solid);
        ...
        const q1 = at(c1, 0.25);
        const rev = [c2[0], ...c2.slice(1).reverse()];
        const dq = (v, w) => Math.hypot(P[v * 3] - P[w * 3], P[v * 3 + 1] - P[w * 3 + 1], P[v * 3 + 2] - P[w * 3 + 2]);
        if (dq(at(rev, 0.25), q1) < dq(at(c2, 0.25), q1))
            c2 = rev;
        const f2 = (() => {
            const f = [0];
        ...
        const na = c1.length, nb = c2.length;
        const fa = (k) => (k >= na ? 1 : f1[k]);
        const fb = (k) => (k >= nb ? 1 : f2[k]);
        let ia = 0, ib = 0;
        while (ia < na || ib < nb) {
            if (ia < na && (ib >= nb || fa(ia) < fb(ib))) {
            ...
            faceOfTri.push(A.fid);
            solidOfTri.push(A.solid);
        }
        sewn++;
        if (DBG)
            console.error(`[tess] sewSolids: ring(${A.v.length}, solid ${A.solid}) <-> ring(${B.v.length}, solid ${B.solid}) worst=${bScore.toExponential(2)}`);
    }
    for (let i = 0; i < rings.length; i++)
        if (!paired.has(i))
            unsewn?.add(rings[i].solid);
    return sewn;
}
function fillMicroHoles(P, I, faceOf, tol, big) {
    const KEY = 2 ** 26;
    const ek = (a, b) => (a < b ? a * KEY + b : b * KEY + a);
    const use = new Map();
    for (let i = 0; i < I.length; i += 3)
    ...
    const multi = new Set(); // boundary vertices with >1 outgoing edge — walking is ambiguous
    for (let i = 0; i < I.length; i += 3)
        for (let e = 0; e < 3; e++) {
            const a = I[i + e], b = I[i + (e + 1) % 3];
            if (use.get(ek(a, b)) !== 1)
            ...
            if (nxt.has(a))
                multi.add(a);
            nxt.set(a, b);
            faceAt.set(a, faceOf[i / 3]);
        }
    const planarDev = (ring) => {
        ...
        let mx = 0;
        for (const v of ring)
            mx = Math.max(mx, Math.abs((P[v * 3] - cx) * nx + (P[v * 3 + 1] - cy) * ny + (P[v * 3 + 2] - cz) * nz));
        return mx;
    };
    ...
    const outI = [], outF = [];
    const seen = new Set();
    const filled = new Set(); // vertices of rings actually filled (seen also marks rejected walks)
    for (const start of nxt.keys()) {
        if (seen.has(start))
            continue;
        const ring = [];
        ...
        for (let g = 0; g <= 64; g++) {
            ...
            if (cur === start)
                break;
            if (g === 64 || seen.has(cur)) {
                ok = false;
                break;
            }
        }
        if (!ok || cur !== start || ring.length < 3 || ring.some((v) => multi.has(v))) {
            if (DBG && ring.length <= 10)
                console.error(`[fill] ring rejected: ok=${ok} closed=${cur === start} len=${ring.length} multi=${ring.some((v) => multi.has(v))} per=${per.toFixed(3)}`);
            continue;
        }
        const micro = per <= tol;
        ...
        if (!micro && !patch) {
            if (DBG)
                console.error(`[fill] ring not filled: len=${ring.length} per=${per.toFixed(3)} dev=${planarDev(ring).toExponential(2)}`);
            continue;
        }
    ...
    if (big) {
        const dirOpen = new Set(); // a->b as traversed by the owning triangle (winding vote)
        const edges = [];
        const inc = new Map(); // vertex -> incident open-edge ids
        for (let i = 0; i < I.length; i += 3)
            for (let e = 0; e < 3; e++) {
                const a = I[i + e], b = I[i + (e + 1) % 3];
                if (use.get(ek(a, b)) !== 1)
                    continue;
                dirOpen.add(a * KEY + b);
                if (filled.has(a) || filled.has(b))
                    continue; // already closed by the directed pass
        ...
        const unusedAt = (v) => {
            ...
            while (c < list.length && edges[list[c]].used)
                c++;
            cursor.set(v, c);
        ...
        for (let seed = 0; seed < edges.length; seed++) {
            if (edges[seed].used)
                continue;
            const path = [edges[seed].a];
            const at = new Map([[edges[seed].a, 0]]);
            let cur = edges[seed].a;
            for (let guard = 0; guard <= edges.length; guard++) {
                const eid = unusedAt(cur);
                if (eid < 0)
                    break; // dead end (odd-degree residue) — leftover path stays open
                const E = edges[eid];
                E.used = true;
                const next = E.a === cur ? E.b : E.a;
                if (!at.has(next)) {
                    path.push(next);
                    at.set(next, path.length - 1);
                    cur = next;
                    continue;
                }
                ...
                for (let k = i0 + 1; k < path.length; k++)
                    at.delete(path[k]);
                path.length = i0 + 1;
                cur = next;
                const n = ring.length;
                if (n < 3)
                    continue; // doubled edge between one vertex pair — nothing to fill
                let per = 0;
                for (let i = 0; i < n; i++)
                    per += Math.hypot(P[ring[i] * 3] - P[ring[(i + 1) % n] * 3], P[ring[i] * 3 + 1] - P[ring[(i + 1) % n] * 3 + 1], P[ring[i] * 3 + 2] - P[ring[(i + 1) % n] * 3 + 2]);
                const okFill = per <= tol || (n <= MAXN && per <= perCap && (n === 3 || planarDev(ring) <= Math.max(FLAT * per, big.dev)));
                if (!okFill) {
                    if (DBG)
                        console.error(`[fill] undirected cycle not filled: len=${n} per=${per.toFixed(3)} dev=${planarDev(ring).toExponential(2)}`);
                    continue;
                }
                let agree = 0;
                for (let i = 0; i < n; i++)
                    if (dirOpen.has(ring[i] * KEY + ring[(i + 1) % n]))
                        agree++;
                ...
                for (const v of ring)
                    filled.add(v);
                if (DBG)
                    console.error(`[fill] undirected ring filled: len=${n} per=${per.toFixed(3)}`);
            }
        }
    }
 ...
 */
function triangulateRing(P, ring, use, ek, per, outI, outF, face) {
    const n = ring.length;
    if (n < 3)
        return;
    if (n === 3) {
        outI.push(ring[2], ring[1], ring[0]);
        outF.push(face);
        return;
    }
    const X = (i) => P[ring[i] * 3], Y = (i) => P[ring[i] * 3 + 1], Z = (i) => P[ring[i] * 3 + 2];
    const area = (i, m, j) => {
    ...
    const pen = (i, j) => (j - i <= 1 || (i === 0 && j === n - 1)) ? 0 : (use.has(ek(ring[i], ring[j])) ? 1e6 * per * per : 0);
...
//# sourceMappingURL=tessellate.js.map