...
export function refineTriangulation(pts, tris, boundary, target) {
    ...
    const fatEnough = (t) => area2(pts, tris[t], tris[t + 1], tris[t + 2]) > epsA && altitude(t) > altMin;
    ...
    const splitPass = () => {
        ...
        for (const [, a, b] of cand) {
            ...
            if (ts.some((t) => touched.has(t)))
                continue; // stale this pass — next round catches it
    ...
    const flipPasses = (maxSweeps) => {
        for (let sweep = 0; sweep < maxSweeps; sweep++) {
            ...
            for (const [k, ts] of em) {
                ...
                const has = (t, x) => tris[t] === x || tris[t + 1] === x || tris[t + 2] === x;
                if (!has(t1, a0) || !has(t1, b0) || !has(t2, a0) || !has(t2, b0))
                    continue;
                // orient so t1 traverses a->b (CCW keeps c strictly left, d strictly right)
                const a = a0, b = b0;
                const traverses = (t, x, y) => {
                    for (let e = 0; e < 3; e++)
                        if (tris[t + e] === x && tris[t + (e + 1) % 3] === y)
                            return true;
                    return false;
                };
                if (!traverses(t1, a, b)) {
                ...
                if (!traverses(t1, a, b) || !traverses(t2, b, a))
                    continue; // tangled/degenerate — leave it
                const third = (t, x, y) => {
                    for (let e = 0; e < 3; e++) {
                        const v = tris[t + e];
                        if (v !== x && v !== y)
                ...
                if (!thinCap) {
                    if (area2(pts, a, b, c) <= epsA || area2(pts, b, a, d) <= epsA)
                        continue; // degenerate flap
                    ...
                    const det = (adx * adx + ady * ady) * (bdx * cdy - cdx * bdy) -
                ...
                if (area2(pts, a, d, c) <= epsA || area2(pts, d, b, c) <= epsA)
                    continue;
                const nk = ekey(c, d);
                if (boundary.has(nk) || em.has(nk))
                    continue;
                tris[t1] = a;
                tris[t1 + 1] = d;
                tris[t1 + 2] = c;
    ...
    for (let round = 0; round < 64 && dry < 2; round++) {
        ...
        flipPasses(4);
        smoothPasses(3);
        if (pts.length / 2 > 250_000)
            break; // runaway backstop, far beyond any real face
    }
    flipPasses(8);
    smoothPasses(4);
    flipPasses(4);
