...
function linkedList(data, start, end, clockwise) {
    ...
    if (clockwise === signedArea(data, start, end) > 0) {
        for (let i = start; i < end; i += 2)
            last = insertNode((i / 2) | 0, data[i], data[i + 1], last);
    }
    else {
...
function earcutLinked(earStart, triangles, pass) {
    let ear = earStart;
    if (!ear)
        return;
    let stop = ear;
    ...
    do {
        if (hy <= p.y && hy >= p.next.y && p.next.y !== p.y) {
            ...
            if (x <= hx && x > qx) {
                ...
                if (x === hx)
                    return m; // hole touches the outer directly at a vertex
            }
        }
        p = p.next;
    } while (p !== outerNode);
