import { isBSpline } from "../geom/surfaces.js";
const ekey = (a, b) => (a < b ? a * 0x4000000 + b : b * 0x4000000 + a);
/**
 ...
 */
function localTargetEdge(s, faceId, a, b) {
    const surface = s.surf.get(faceId);
    if (!surface)
        return s.maxEdge;
...
function projectToFace(s, faceId, p) {
    const surface = s.surf.get(faceId);
    if (!surface)
        return p;
...
function classifyVertices(s) {
    ...
    for (let v = 0; v < nv; v++) {
        ...
        if (frozenFace(s, f)) {
            faceOf[v] = -1;
            continue;
        } // keep B-spline / unknown faces as-is
        const surface = s.surf.get(f);
        if (!surface)
            continue;
        const [u, vv] = surface.project([s.pos[v * 3], s.pos[v * 3 + 1], s.pos[v * 3 + 2]]);
        if (surface.curvatureRadius(u, vv) < s.maxEdge / 8)
            faceOf[v] = -1;
    }
    return faceOf;
}
function dist2(s, a, b) {
...
function splitPass(s) {
    if (s.tris.length / 3 >= s.maxTris)
        return false; // triangle budget reached
    const em = edgeTriMap(s);
    const mid = new Map(); // edge key -> new vertex index
    for (const [k, ts] of em) {
        if (ts.length !== 2 || s.tFace[ts[0]] !== s.tFace[ts[1]])
            continue; // feature/boundary edge
        if (frozenFace(s, s.tFace[ts[0]]))
            continue; // unprojectable / kept-as-is face
        const a = Math.floor(k / 0x4000000), b = k % 0x4000000;
        const high = (4 / 3) * localTargetEdge(s, s.tFace[ts[0]], a, b);
        if (dist2(s, a, b) <= high * high)
            continue;
        const p = [
    ...
    const newTris = [];
    const newFace = [];
    const emit = (a, b, c, f) => { newTris.push(a, b, c); newFace.push(f); };
    for (let t = 0; t < nt; t++) {
...
function setTriOriented(s, t, x, y, z) {
    ...
    if (surface) {
        ...
        const nx = uy * vz - uz * vy, ny = uz * vx - ux * vz, nz = ux * vy - uy * vx;
        const cen = [(ax + s.pos[y * 3] + s.pos[z * 3]) / 3, (ay + s.pos[y * 3 + 1] + s.pos[z * 3 + 1]) / 3, (az + s.pos[y * 3 + 2] + s.pos[z * 3 + 2]) / 3];
...
function flipPass(s) {
    ...
    const created = new Set(); // edges introduced this pass (avoid making them non-manifold)
    for (const [k, ts] of em) {
        if (ts.length !== 2)
            continue;
...
function compact(s, deadV, deadT) {
    const nv = s.pos.length / 3;
...
function collapsePass(s) {
    ...
    for (const [k, ts] of em) {
        if (ts.length !== 2)
            continue;
        const ti = ts[0], tj = ts[1];
        if (deadT[ti] || deadT[tj] || s.tFace[ti] !== s.tFace[tj])
            continue;
        if (frozenFace(s, s.tFace[ti]))
            continue; // unprojectable face — keep as-is
        const a = Math.floor(k / 0x4000000), b = k % 0x4000000;
        if (deadV[a] || deadV[b] || touched[a] || touched[b])
            continue;
        const low = (4 / 5) * localTargetEdge(s, s.tFace[ti], a, b);
        if (dist2(s, a, b) >= low * low)
            continue;
        ...
        if (fa >= 0 && fb >= 0) {
            kp = a;
            rm = b;
            pos = projectToFace(s, fa, [(s.pos[a * 3] + s.pos[b * 3]) / 2, (s.pos[a * 3 + 1] + s.pos[b * 3 + 1]) / 2, (s.pos[a * 3 + 2] + s.pos[b * 3 + 2]) / 2]);
        }
        else if (fa < 0 && fb >= 0) {
            kp = a;
            rm = b;
            pos = [s.pos[a * 3], s.pos[a * 3 + 1], s.pos[a * 3 + 2]];
        }
        else if (fb < 0 && fa >= 0) {
            kp = b;
            rm = a;
            pos = [s.pos[b * 3], s.pos[b * 3 + 1], s.pos[b * 3 + 2]];
        }
        else
            continue;
        ...
        const Pw = (i) => (i === kp || i === rm ? pos : vpos(s, i));
        const around = new Set();
        for (const t of v2t[a])
            if (!deadT[t] && t !== ti && t !== tj)
                around.add(t);
        for (const t of v2t[b])
            if (!deadT[t] && t !== ti && t !== tj)
                around.add(t);
        for (const t of around) {
            ...
            if (nl < 1e-12 || (nx * old[0] + ny * old[1] + nz * old[2]) / nl < 0.2) {
                ok = false;
                break;
            }
            ...
            if (Math.max(e1, e2, e3) > highCap2) {
                ok = false;
                break;
            }
            postMin = Math.min(postMin, minAngleP(A, B, C));
        }
        if (!ok || (postMin < preMin - 1e-3 && postMin < 20 * Math.PI / 180))
            continue;
        deadV[rm] = 1;
        deadT[ti] = 1;
        deadT[tj] = 1;
...
function smoothPass(s, faceOf) {
    ...
    const acc = (v, w) => {
        sum[v * 3] += s.pos[w * 3];
        sum[v * 3 + 1] += s.pos[w * 3 + 1];
        sum[v * 3 + 2] += s.pos[w * 3 + 2];
        cnt[v]++;
    };
    for (let t = 0; t < nt; t++) {
        const a = s.tris[t * 3], b = s.tris[t * 3 + 1], c = s.tris[t * 3 + 2];
...
export function remesh(mesh, faceOfTri, surf, opts) {
    ...
    for (let it = 0; it < iterations; it++) {
        ...
        if (!noSmooth)
            smoothPass(s, faceOf);
        const nt = s.tris.length / 3;
        if (it > 2 && (!changed || Math.abs(nt - prevTris) < 0.02 * prevTris))
            break;
        prevTris = nt;
    }
    if (dbg)
        console.log(`  remesh iters=${iters}`);
    if (!noFlip)
        flipPass(s); // final de-sliver
    return {
