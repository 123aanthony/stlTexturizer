import { Table, ref, refList, enumOf, num } from "../step/entities.js";
import { parseStep } from "../step/parser.js";
import { detectUnits, detectUncertainty, contextLengthScales } from "../step/units.js";
import { assemblyLinks } from "../step/assembly.js";
import { readPoint, readPlacement } from "../geom/placement.js";
import { makeCurve } from "../geom/curves.js";
...
const applyF = (f, p) => { const r = rot(f, p); return [r[0] + f.o[0], r[1] + f.o[1], r[2] + f.o[2]]; };
const composeF = (a, b) => ({ o: applyF(a, b.o), x: rot(a, b.x), y: rot(a, b.y), z: rot(a, b.z) });
const invF = (f) => {
    const t = { o: [0, 0, 0], x: [f.x[0], f.y[0], f.z[0]], y: [f.x[1], f.y[1], f.z[1]], z: [f.x[2], f.y[2], f.z[2]] };
    const o = rot(t, f.o);
    t.o = [-o[0], -o[1], -o[2]];
    return t;
};
 ...
 */
function assemblyInfo(t, s) {
    // geometry rep (ABSR / SHAPE_REPRESENTATION / ...) -> the solid bodies it contains
    const repOfSolid = new Map();
    const repCtx = new Map();
    for (const ty of ["ADVANCED_BREP_SHAPE_REPRESENTATION", "MANIFOLD_SURFACE_SHAPE_REPRESENTATION", "SHAPE_REPRESENTATION"]) {
        for (const [repId, rep] of t.byType(ty)) {
            if (rep.params[2]?.k === "ref")
                repCtx.set(repId, ref(rep.params[2]));
            if (rep.params[1]?.k !== "list")
                continue;
            for (const item of refList(rep.params[1])) {
                const ty2 = t.typeOf(item);
                if (ty2 === "MANIFOLD_SOLID_BREP" || ty2 === "BREP_WITH_VOIDS" || ty2 === "SHELL_BASED_SURFACE_MODEL")
                    repOfSolid.set(item, repId);
            }
        }
    }
    const ctxScale = contextLengthScales(t);
    const scaleOfRep = (rep) => {
    ...
    for (const l of assemblyLinks(t)) {
        if (l.childItem === undefined || l.parentItem === undefined)
            continue;
    ...
    for (const [id, srr] of t.byType("SHAPE_REPRESENTATION_RELATIONSHIP")) {
        if (t.isComplex(id) || srr.params[2]?.k !== "ref" || srr.params[3]?.k !== "ref")
            continue;
        const a = ref(srr.params[2]), b = ref(srr.params[3]);
        equiv.set(a, b);
        equiv.set(b, a);
    }
    const resolve = (rep) => (parents.has(rep) ? rep : (equiv.get(rep) ?? rep));
    ...
    const solidScale = new Map();
    for (const [solidId, geomRep] of repOfSolid) {
        const worlds = worldsOf(geomRep);
        if (worlds.length > 1 || worlds[0] !== IDENT)
            instances.set(solidId, worlds);
        const sc = scaleOfRep(geomRep);
        if (sc !== s)
            solidScale.set(solidId, sc);
    }
    return { instances, solidScale };
}
const vertexPoint = (t, id, s) => readPoint(t, ref(t.record(id).params[1]), s); // VERTEX_POINT(name, point#)
/**
 ...
 */
function healMicroEdges(table, edges, solids, sGlobal) {
    ...
    for (const [id, e] of edges) {
        if (table.typeOf(id) !== "EDGE_CURVE")
            continue;
        const rec = table.record(id);
        if (rec.params[1]?.k !== "ref" || rec.params[2]?.k !== "ref")
            continue;
    ...
    for (const [id, [va, vb]] of vids) {
        if (va === vb)
            continue; // closed edge on one shared vertex — a rim, not a bridge
        const e = edges.get(id);
        const d = d3(e.v0, e.v1);
        if (d >= tol)
            continue;
        const { id: cid, kind } = unwrapCurve(e.curveId);
        if (kind !== "LINE" && kind !== "CIRCLE" && kind !== "ELLIPSE")
            continue;
    ...
    if (candidates.length === 0)
        return; // clean topology — bit-identical fast path
    ...
    }
    {
        ...
        for (const [vid, p] of vpos) {
            ...
            if (!l || !h) {
                lo.set(r, [p[0], p[1], p[2]]);
                hi.set(r, [p[0], p[1], p[2]]);
                continue;
            }
            for (let k = 0; k < 3; k++) {
                if (p[k] < l[k])
        ...
        for (const [r, l] of lo) {
            const h = hi.get(r);
            if (Math.hypot(h[0] - l[0], h[1] - l[1], h[2] - l[2]) > 2 * tol)
                poisoned.add(r);
        }
    }
    ...
    const arcExtent = (e, curveId, kind) => {
    ...
    for (const { id, kind, cid } of candidates) {
        if (poisoned.has(find(vids.get(id)[0])))
            continue;
        const e = edges.get(id);
        if (kind === "LINE") {
            drop.add(id);
    ...
    if (dirty.size === 0 && drop.size === 0)
        return;
    for (const [id, [va, vb]] of vids) {
        const e = edges.get(id);
        const ra = find(va), rb = find(vb);
        if (ra !== va && dirty.has(ra)) {
            const p = vpos.get(ra);
            e.v0 = [p[0], p[1], p[2]];
        }
        if (rb !== vb && dirty.has(rb)) {
            const p = vpos.get(rb);
            e.v1 = [p[0], p[1], p[2]];
        }
    }
    ...
    for (const solid of solids) {
        ...
        for (const face of solid.faces) {
            ...
            for (const loop of face.loops) {
                if (!loop.edges.some((oe) => drop.has(oe.edgeId)))
                    continue;
        ...
        if (gone.size > 0)
            solid.faces = solid.faces.filter((f) => !gone.has(f));
    }
}
export function buildBrep(src) {
    ...
    const resolveShellFaces = (shellId, flip) => {
    ...
    const buildFace = (fid, flip) => {
        ...
        for (const bId of refList(f.params[1])) {
            ...
            if (edges.length > 0)
                loops.push({ outer: b.type === "FACE_OUTER_BOUND", edges });
        }
        return {
    ...
    const solids = [];
    const droppedFaces = [];
    const addSolid = (sid, shellIds, open = false) => {
        const faces = [];
        ...
        for (const shellId of shellIds)
            for (const sf of resolveShellFaces(shellId, false)) {
                try {
                    faces.push(buildFace(sf.fid, sf.flip));
                }
                catch {
                    droppedFaces.push(sf.fid);
                }
            }
        if (faces.length > 0)
            solids.push(open ? { id: sid, faces, open } : { id: sid, faces });
    };
    // MANIFOLD_SOLID_BREP(name, outer_shell); BREP_WITH_VOIDS(name, outer_shell, (void_shells)).
    for (const [sid, msb] of table.byType("MANIFOLD_SOLID_BREP"))
        addSolid(sid, [ref(msb.params[1])]);
    for (const [sid, bwv] of table.byType("BREP_WITH_VOIDS"))
        addSolid(sid, [ref(bwv.params[1]), ...refList(bwv.params[2])]);
        ...
        for (const [cbsId, cbs] of table.byType("CURVE_BOUNDED_SURFACE")) {
            try {
                ...
                for (const bId of refList(cbs.params[2])) {
                    ...
                    if (oedges.length > 0)
                        loops.push({ outer: b.type === "OUTER_BOUNDARY_CURVE", edges: oedges });
                }
                if (loops.length > 0) {
    ...
    for (const solid of solids) {
        ...
        if (inst) {
            solid.instances = inst;
            solid.transform = inst[0];
        }
