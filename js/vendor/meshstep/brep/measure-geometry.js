import { analyzeEdgeCurve } from "../geom/curves.js";
import { analyzeSurface } from "../geom/surfaces.js";
 ...
 */
export function collectMeasureGeometry(brep, edgePolylines, instances) {
    const edges = [];
    const faces = [];
    const raw = [];
    let edgeInstances = 0;
        ...
        for (const face of solid.faces) {
            ...
            for (const f of frames) {
                faces.push({
                    ...
                    origin: surfInfo.origin ? applyF(f, surfInfo.origin) : undefined,
                    axis: surfInfo.axis ? rot(f, surfInfo.axis) : undefined,
            ...
            for (const lp of face.loops)
                for (const oe of lp.edges) {
                    ...
                    if (list) {
                        if (!list.includes(face.faceId))
                            list.push(face.faceId);
                    }
                    else
                        edgeFaces.set(oe.edgeId, [face.faceId]);
                }
        }
        for (const [edgeId, faceIds] of edgeFaces) {
            const e = brep.edges.get(edgeId);
            const polyline = edgePolylines.get(edgeId);
            if (!e || !polyline || polyline.length < 2)
                continue; // healed-away micro-edge
    ...
    if (truncated) {
        for (const r of raw)
            if (r.info.kind === "other")
                r.polyline = decimate(r.polyline, DECIMATED_POINTS);
    }
    // Pass 2: pack instance-placed polylines and emit final edge records.
    let totalPts = 0;
    for (const r of raw)
        totalPts += r.polyline.length * r.frames.length;
    ...
    for (const r of raw) {
        for (const f of r.frames) {
            ...
            edges.push({
                ...
                center: r.info.center ? applyF(f, r.info.center) : undefined,
                axis: r.info.axis ? rot(f, r.info.axis)
                    : r.info.dir ? rot(f, r.info.dir) : undefined,
...
//# sourceMappingURL=measure-geometry.js.map