...
import { Table, ref, refList, list, num, str } from "./entities.js";
...
const resolveColour = (t, id) => {
    const rgb = t.sub(id, "COLOUR_RGB");
    if (rgb)
        return [num(rgb.params[1]), num(rgb.params[2]), num(rgb.params[3])];
    ...
    for (const s of list(psa.params[0])) {
        ...
        for (const el of refList(side.params[1])) {
            ...
            if (fill) {
                ...
                if (fas)
                    for (const item of refList(fas.params[1])) {
            ...
            if (rend && rend.params[1]?.k === "ref")
                rendering ??= resolveColour(t, ref(rend.params[1]));
        }
        if (rendering)
            return rendering;
    }
 ...
 */
export function extractColors(table, solids) {
    const faceRaw = new Map();
    const solidRaw = new Map();
    const put = (m, id, rank, rgb) => {
        const cur = m.get(id);
        if (!cur || rank > cur.rank)
            m.set(id, { rank, rgb });
    };
    const unwrapFace = (id) => {
        while (table.typeOf(id) === "ORIENTED_FACE")
            id = ref(table.record(id).params[2]);
        return id;
    };
    const shellFaces = (id) => {
        const or = table.sub(id, "ORIENTED_CLOSED_SHELL");
        if (or)
            return shellFaces(ref(or.params[2]));
    ...
    const apply = (rec, overriding) => {
        if (rec.params[1]?.k !== "list" || rec.params[2]?.k !== "ref")
            return;
        let rgb = null;
        for (const s of list(rec.params[1])) {
            if (s.k === "ref")
                rgb = colourOfPSA(table, s.v);
            if (rgb)
                break;
        }
        if (!rgb)
        ...
        switch (table.typeOf(item)) {
            ...
            case "ORIENTED_CLOSED_SHELL":
                for (const f of shellFaces(item))
                    put(faceRaw, unwrapFace(f), 1 + bump, rgb);
                break;
    ...
    for (const [, rec] of table.byType("STYLED_ITEM")) {
        try {
            apply(rec, false);
        }
        catch { /* skip malformed */ }
    }
    for (const [, rec] of table.byType("OVER_RIDING_STYLED_ITEM")) {
        try {
            apply(rec, true);
        }
        catch { /* skip malformed */ }
    }
    if (faceRaw.size === 0 && solidRaw.size === 0)
        return null;
    const palette = [];
    const index = new Map();
    const idx = (c) => {
    ...
    for (const solid of solids) {
        ...
        if (sc)
            solidColor.set(solid.id, idx(sc.rgb));
        for (const face of solid.faces) {
            const fc = faceRaw.get(face.faceId) ?? sc;
            if (fc)
                faceColor.set(face.faceId, idx(fc.rgb));
        }
    }
    if (faceColor.size === 0 && solidColor.size === 0)
        return null;
