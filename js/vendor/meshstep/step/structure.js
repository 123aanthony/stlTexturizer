...
import { Table, ref, refList, str } from "./entities.js";
import { assemblyLinks } from "./assembly.js";
 ...
 */
export function extractStructure(table, solids) {
    const bodyName = (id) => {
        for (const ty of SOLID_TYPES) {
            const rec = table.sub(id, ty);
            if (rec?.params[0]?.k === "str")
                return rec.params[0].v;
        }
        return "";
    };
    const fallback = () => ({
...
function buildTree(table, solids, bodyName) {
    ...
    for (const [, sdr] of table.byType("SHAPE_DEFINITION_REPRESENTATION")) {
        try {
            if (sdr.params[0]?.k !== "ref" || sdr.params[1]?.k !== "ref")
                continue;
    ...
    const childrenOf = new Map(); // parent -> child -> occurrence count
    const hasParent = new Set();
    const equiv = new Map();
    for (const [id, srr] of table.byType("SHAPE_REPRESENTATION_RELATIONSHIP")) {
        if (table.isComplex(id) || srr.params[2]?.k !== "ref" || srr.params[3]?.k !== "ref")
            continue;
        const a = ref(srr.params[2]), b = ref(srr.params[3]);
        equiv.set(a, b);
        equiv.set(b, a);
    }
    const links = assemblyLinks(table);
    ...
    const linkedReps = new Set();
    for (const l of links) {
        linkedReps.add(l.child);
        linkedReps.add(l.parent);
    }
    const resolve = (rep) => (linkedReps.has(rep) ? rep : (equiv.get(rep) ?? rep));
    for (const l of links) {
    ...
    const build = (rep, occurrences, onPath) => {
        placed.add(rep);
        const children = [];
        const kids = childrenOf.get(rep);
        if (kids && !onPath.has(rep)) {
            onPath.add(rep);
            for (const [child, count] of kids)
                children.push(build(child, count, onPath));
            onPath.delete(rep);
        }
