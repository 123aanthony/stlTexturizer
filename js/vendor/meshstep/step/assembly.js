...
export function assemblyLinks(t) {
    ...
    const nauoOfRel = new Map();
    for (const [, cdsr] of t.byType("CONTEXT_DEPENDENT_SHAPE_REPRESENTATION")) {
        if (cdsr.params[0]?.k !== "ref" || cdsr.params[1]?.k !== "ref")
            continue;
        ...
        if (nauo?.params[3]?.k !== "ref" || nauo.params[4]?.k !== "ref")
            continue;
    ...
    for (const [id, rrwt] of t.byType("REPRESENTATION_RELATIONSHIP_WITH_TRANSFORMATION")) {
        ...
        if (!rr || rr.params[2]?.k !== "ref" || rr.params[3]?.k !== "ref")
            continue;
        ...
        if (nauo && nauo.parentPd !== nauo.childPd) {
            ...
            if (pd1 === nauo.parentPd || pd2 === nauo.childPd)
                swapped = true;
        }
        links.push(swapped
