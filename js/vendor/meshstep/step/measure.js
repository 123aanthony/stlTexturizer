...
import { ref, num } from "./entities.js";
import { readPlacement } from "../geom/placement.js";
class Box {
    ...
    get empty() { return this.min[0] === Infinity; }
}
const applyFrame = (f, x, y, z) => [
...
export function estimateStepSize(src) {
    try {
        ...
        for (const solid of brep.solids) {
            ...
            for (const face of solid.faces)
                for (const lp of face.loops)
                    for (const oe of lp.edges) {
                        ...
                        if (e.v0[0] !== e.v1[0] || e.v0[1] !== e.v1[1] || e.v0[2] !== e.v1[2])
                            continue;
                        try {
            ...
            for (const f of frames)
                for (let c = 0; c < 8; c++) {
                    const p = applyFrame(f, (c & 1 ? local.max : local.min)[0], (c & 2 ? local.max : local.min)[1], (c & 4 ? local.max : local.min)[2]);
 ...
 */
export function autoTessellation(diagMm) {
