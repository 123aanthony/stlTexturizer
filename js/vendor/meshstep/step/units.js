import { num, ref } from "./entities.js";
const SI_PREFIX = {
...
function siFactor(si, kind) {
    const nameP = si.params[1];
    const name = nameP && nameP.k === "enum" ? nameP.v : "";
    if (kind === "length" && name !== "METRE")
        return null;
    if (kind === "angle" && name !== "RADIAN")
        return null;
 ...
 */
function unitFactor(t, id, kind, depth = 0) {
    if (depth > 4)
        return null;
    const si = t.sub(id, "SI_UNIT");
    if (si)
        return siFactor(si, kind);
...
export function detectUncertainty(table) {
    let best = null;
...
function lengthAbbrev(mmPerUnit, cbuName) {
    ...
    for (const [f, a] of byFactor)
        if (Math.abs(mmPerUnit - f) <= 1e-9 * Math.max(1, f))
            return a;
    return `${+mmPerUnit.toPrecision(6)} mm/unit`;
}
export function detectUnits(table) {
    let mmPerUnit = null;
    let radPerAngle = null;
    let label = "";
    const takeUnit = (id) => {
