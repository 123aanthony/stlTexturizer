...
const sharedGLSL = /* glsl */`
  ...
  vec3 woodWeights(float a, float b, float c) {
    vec3 q = vec3(a, b, c);
    q *= q;
    q *= q;
    return q / (dot(q, vec3(1.0)) + 1e-6);
  }

  float woodAutoHeight(vec3 pos, vec3 woodN, float md) {
    vec3 absN = abs(woodN);

    // Longest AABB axis = intended grain direction.
    // The grain U coordinate stays on that axis. The V coordinate blends between
    // the two cross-section axes so hard top/side edges don't snap projection.
    if (boundsSize.x >= boundsSize.y && boundsSize.x >= boundsSize.z) {
      float rawU = (pos.x - boundsMin.x) / md;
      vec3 w = woodWeights(absN.x, absN.y, absN.z);
      float hEnd = sampleMap(vec2((pos.y - boundsMin.y) / md, (pos.z - boundsMin.z) / md));
      float hY   = sampleMap(vec2(rawU, (pos.z - boundsMin.z) / md));
      float hZ   = sampleMap(vec2(rawU, (pos.y - boundsMin.y) / md));
      return hEnd * w.x + hY * w.y + hZ * w.z;

    } else if (boundsSize.y >= boundsSize.x && boundsSize.y >= boundsSize.z) {
      float rawU = (pos.y - boundsMin.y) / md;
      vec3 w = woodWeights(absN.y, absN.x, absN.z);
      float hEnd = sampleMap(vec2((pos.x - boundsMin.x) / md, (pos.z - boundsMin.z) / md));
      float hX   = sampleMap(vec2(rawU, (pos.z - boundsMin.z) / md));
      float hZ   = sampleMap(vec2(rawU, (pos.x - boundsMin.x) / md));
      return hEnd * w.x + hX * w.y + hZ * w.z;

    } else {
      float rawU = (pos.z - boundsMin.z) / md;
      vec3 w = woodWeights(absN.z, absN.x, absN.y);
      float hEnd = sampleMap(vec2((pos.x - boundsMin.x) / md, (pos.y - boundsMin.y) / md));
      float hX   = sampleMap(vec2(rawU, (pos.y - boundsMin.y) / md));
      float hY   = sampleMap(vec2(rawU, (pos.x - boundsMin.x) / md));
      return hEnd * w.x + hX * w.y + hY * w.z;
    }
  }
  ...
  float computeHeightAtPoint(vec3 pos, vec3 projN, vec3 blendN) {
    ...
    if (mappingMode == 0) {
      ...
      return woodAutoHeight(pos, blendN, md);
...
const vertexShader = /* glsl */`
  ...
  varying vec3  vModelSmoothNormal; // model-space smooth normal → Wood Auto edge blending
...
const fragmentShader = /* glsl */`
  ...
  varying vec3  vModelSmoothNormal;
  ...
  float getHeight() {
    ...
    vec3 woodN = length(vModelSmoothNormal) > 1e-6 ? normalize(vModelSmoothNormal) : vModelNormal;
    return computeHeightAtPoint(vModelPos, PN, mappingMode == 7 ? woodN : vModelNormal);
  }
