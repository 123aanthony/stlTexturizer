...
export const MODE_WOOD_BEAM_MANUAL_V3 = 11;
...
const sharedGLSL = /* glsl */`
  ...
  float sampleMapNoRotation(vec2 rawUV) {
    vec2 uv = (rawUV * textureAspect) / scaleUV + offsetUV;
    return texture2D(displacementMap, uv).r;
  }

  int woodAxisForMode() {
    if (mappingMode == 8) return 0; // Wood X
    if (mappingMode == 9) return 1; // Wood Y
    if (mappingMode == 10) return 2; // Wood Z

    // Wood Auto: longest AABB axis = intended grain direction.
    if (boundsSize.y >= boundsSize.x && boundsSize.y >= boundsSize.z) return 1;
    if (boundsSize.z >= boundsSize.x && boundsSize.z >= boundsSize.y) return 2;
    return 0;
  }
  ...
  float woodBeamHeight(vec3 pos, vec3 projN, float md) {
    ...
    float rawU = 0.0;
    float rawV = 0.0;
    int axis = woodAxisForMode();

    if (axis == 0) {
      rawU = (pos.x - boundsMin.x) / md;
      if (absN.x >= absN.y && absN.x >= absN.z) rawV = (pos.y - boundsMin.y) / md;
      else if (absN.z >= absN.y) rawV = (pos.y - boundsMin.y) / md;
      else rawV = (pos.z - boundsMin.z) / md;

    } else if (axis == 1) {
      rawU = (pos.y - boundsMin.y) / md;
      if (absN.y >= absN.x && absN.y >= absN.z) rawV = (pos.x - boundsMin.x) / md;
      else if (absN.z >= absN.x) rawV = (pos.x - boundsMin.x) / md;
      else rawV = (pos.z - boundsMin.z) / md;

    } else {
      rawU = (pos.z - boundsMin.z) / md;
      if (absN.z >= absN.x && absN.z >= absN.y) rawV = (pos.x - boundsMin.x) / md;
      else if (absN.y >= absN.x) rawV = (pos.x - boundsMin.x) / md;
      else rawV = (pos.y - boundsMin.y) / md;
    }

    return sampleMap(vec2(rawU, rawV));
  }

  float woodBeamManualV3Height(vec3 pos, vec3 projN, float md) {
    float c = cos(rotation);
    float s = sin(rotation);

    vec2 relXY = pos.xy - boundsCenter.xy;
    float relZ = pos.z - boundsCenter.z;

    vec2 beamDir = vec2(c, s);
    vec2 crossDir = vec2(-s, c);

    float along = dot(relXY, beamDir);
    float across = dot(relXY, crossDir);
    float height = relZ;

    float nAlong = abs(projN.x * c + projN.y * s);
    float nAcross = abs(projN.x * (-s) + projN.y * c);
    float nUp = abs(projN.z);

    float rawU;
    float rawV;

    if (nUp >= nAlong && nUp >= nAcross) {
      // Top / bottom of beam.
      rawU = along / md;
      rawV = across / md;
    } else if (nAcross >= nAlong) {
      // Long side of beam.
      rawU = along / md;
      rawV = height / md;
    } else {
    ...
    // Rotation has already been consumed as the beam frame.
    return sampleMapNoRotation(vec2(rawU, rawV));
  }
  ...
  float computeHeightAtPoint(vec3 pos, vec3 projN, vec3 blendN) {
    ...
    } else if (mappingMode >= 7 && mappingMode <= 10) {
      return woodBeamHeight(pos, projN, md);

    } else if (mappingMode == 11) {
      return woodBeamManualV3Height(pos, projN, md);
