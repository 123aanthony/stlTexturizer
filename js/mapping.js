...
export function computeUV(pos, normal, mode, settings, bounds) {
  ...
  // Wood Beam Manual V3:
  // Rotation is used as the beam axis angle in the XY plane.
  // Faces are classified in the LOCAL beam frame:
  // - top/bottom: U = beam length, V = beam width
  // - long side:  U = beam length, V = height
  // - end cap:    U = beam width,  V = height
  // This avoids the world XYZ normal switching that breaks inclined beams.
  function computeWoodBeamManualV3UV() {
    const dx = cosR;
    const dy = sinR;
    const px = -sinR;
    const py = cosR;
    ...
    const nAlong = Math.abs(normal.x * dx + normal.y * dy);
    const nAcross = Math.abs(normal.x * px + normal.y * py);
    const nUp = Math.abs(normal.z);
    ...
    // Rotation is already consumed as the beam frame. Do not rotate UV again.
    return applyTransform(rawU, rawV, scaleU, scaleV, offsetU, offsetV, 1, 0);
  }
  ...
  switch (mode) {
    ...
    case MODE_WOOD_BEAM_MANUAL_V3: {
      return computeWoodBeamManualV3UV();
    }
