...
export function computeUV(pos, normal, mode, settings, bounds) {
  ...
  // V is blended between the two cross-section axes using the supplied normal.
  // This softens the top/side edge instead of snapping from one projection to the other.
  function _woodWeights(a, b, c) {
    const aa = a * a, bb = b * b, cc = c * c;
    const wa = aa * aa;
    const wb = bb * bb;
    const wc = cc * cc;
    const sum = wa + wb + wc + 1e-6;
    return { a: wa / sum, b: wb / sum, c: wc / sum };
  }
  ...
  function computeWoodAutoUV() {
    ...
    if (sx >= sy && sx >= sz) {
      const rawU = (pos.x - min.x) / md;
      const w = _woodWeights(nx, ny, nz);
      const endCap = applyTransform((pos.y - min.y) / md, (pos.z - min.z) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      const sideY  = applyTransform(rawU, (pos.z - min.z) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      const sideZ  = applyTransform(rawU, (pos.y - min.y) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      return {
        triplanar: true,
        samples: [
          { u: endCap.u, v: endCap.v, w: w.a },
          { u: sideY.u,  v: sideY.v,  w: w.b },
          { u: sideZ.u,  v: sideZ.v,  w: w.c },
        ],
      };
    }

    if (sy >= sx && sy >= sz) {
      const rawU = (pos.y - min.y) / md;
      const w = _woodWeights(ny, nx, nz);
      const endCap = applyTransform((pos.x - min.x) / md, (pos.z - min.z) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      const sideX  = applyTransform(rawU, (pos.z - min.z) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      const sideZ  = applyTransform(rawU, (pos.x - min.x) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
      return {
        triplanar: true,
        samples: [
          { u: endCap.u, v: endCap.v, w: w.a },
          { u: sideX.u,  v: sideX.v,  w: w.b },
          { u: sideZ.u,  v: sideZ.v,  w: w.c },
        ],
      };
    }

    const rawU = (pos.z - min.z) / md;
    const w = _woodWeights(nz, nx, ny);
    const endCap = applyTransform((pos.x - min.x) / md, (pos.y - min.y) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
    const sideX  = applyTransform(rawU, (pos.y - min.y) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
    const sideY  = applyTransform(rawU, (pos.x - min.x) / md, scaleU, scaleV, offsetU, offsetV, cosR, sinR);
    return {
      triplanar: true,
      samples: [
        { u: endCap.u, v: endCap.v, w: w.a },
        { u: sideX.u,  v: sideX.v,  w: w.b },
        { u: sideY.u,  v: sideY.v,  w: w.c },
      ],
    };
  }
