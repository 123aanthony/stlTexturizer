...
function getNextTextureSlotIndex() {
  const used = new Set(
    textureSlots
      .map(s => String(s.id || '').match(/^slot(\d+)$/)?.[1])
      .filter(Boolean)
      .map(Number)
  );
...
function customTextureLibraryKey(file) {
  return `${file.name}::${file.size}::${file.lastModified || 0}`;
}

function drawEntryThumbnail(entry, size = 64) {
  ...
  if (entry?.fullCanvas) {
    const sw = entry.fullCanvas.width;
    const sh = entry.fullCanvas.height;
    const scale = Math.min(size / sw, size / sh);
    const dw = sw * scale;
    const dh = sh * scale;
    ctx.drawImage(entry.fullCanvas, (size - dw) / 2, (size - dh) / 2, dw, dh);
  }
...
function activateCustomLibraryEntry(entry) {
  if (!entry) return;
...
async function addFilesToCustomTextureLibrary(files) {
  const imageFiles = Array.from(files || []).filter(isSupportedTextureFile);
  if (!imageFiles.length) return;

  for (const file of imageFiles) {
    const key = customTextureLibraryKey(file);
    ...
    try {
      ...
      console.warn('Could not load custom texture:', file.name, err);
    }
  }

  customTextureLibrary.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));
  renderCustomTextureLibrary();
}
...
async function importTextureFolder() {
  ...
  try {
    ...
    for await (const [, handle] of dir.entries()) {
      ...
      if (isSupportedTextureFile(file)) files.push(file);
    }

    await addFilesToCustomTextureLibrary(files);
  } catch (err) {
    if (err?.name === 'AbortError') return;
    console.error('Could not import texture folder:', err);
    alert(`Could not import texture folder: ${err.message}`);
  }
}
...
function _ensureProjectExtension(filePath) {
  if (!filePath) return filePath;
  return /\.(bforge|bumpmesh)$/i.test(filePath) ? filePath : `${filePath}.bforge`;
}

function _projectFilters() {
  return [
    { name: 'BumpForge Project', extensions: ['bforge', 'bumpmesh'] },
    { name: 'All files', extensions: ['*'] }
  ];
}

function _suggestProjectName() {
  const base = (currentStlName || currentProjectDisplayName || 'bumpforge')
    .replace(/\.(bforge|bumpmesh)$/i, '')
    .replace(/[\\/:*?"<>|]+/g, '_')
    .trim() || 'bumpforge';
...
function updateProjectChrome() {
  const chip = document.getElementById('project-title-chip');
  const label = currentProjectPath ? _basenameFromPath(currentProjectPath) : currentProjectDisplayName || 'Untitled';
  const title = `BumpForge - ${label}${projectDirty ? ' *' : ''}`;
  ...
  if (chip) {
    ...
    chip.title = currentProjectPath || label;
    chip.classList.toggle('dirty', projectDirty);
  }
}
...
// ── Project file workflow: New / Open / Save / Save As ───────────────────────
...
function buildProjectPayload({ includeModel = true, includeTexture = true } = {}) {
  saveActiveSlotState();

  const customSource = (_lastCustomMap && _lastCustomMap.fullCanvas) ? _lastCustomMap : null;
  const shouldIncludeTexture = includeTexture && !!customSource;
  ...
  // even if the user has a preset selected at save time.
  if (shouldIncludeTexture) payload.activeMapName = customSource.name;

  const zipFiles = { 'settings.json': strToU8(JSON.stringify(payload, null, 2)) };

  if (includeModel && currentGeometry) {
...
async function buildProjectBytes() {
  const { zipFiles, customSource, shouldIncludeTexture } = buildProjectPayload({
    includeModel: true,
    includeTexture: true
  });

  if (shouldIncludeTexture) {
...
async function saveProjectToPath(filePath) {
  filePath = _ensureProjectExtension(filePath);
  const zipped = await buildProjectBytes();

  if (window.bumpforgeElectron?.writeFile) {
    const result = await window.bumpforgeElectron.writeFile({
      filePath,
      data: zipped.buffer.slice(zipped.byteOffset, zipped.byteOffset + zipped.byteLength)
    });
...
async function _saveProjectAsFlow() {
  if (window.bumpforgeElectron?.saveFile) {
    const result = await window.bumpforgeElectron.saveFile({
      ...
      defaultPath: currentProjectPath || _suggestProjectName(),
      filters: _projectFilters()
    });

    if (result?.canceled || !result?.filePath) return { canceled: true };
    return saveProjectToPath(result.filePath);
  }
...
async function openProjectFromPath(filePath) {
  if (!filePath) return;
  if (window.bumpforgeElectron?.readFile) {
    const result = await window.bumpforgeElectron.readFile({ filePath });
    if (result?.error) throw new Error(result.error);
    const bytes = new Uint8Array(result.data);
    const file = new File([bytes], _basenameFromPath(filePath), { type: 'application/octet-stream' });
    await importProject(file, { filePath });
    return;
  }

  throw new Error('Native file read is only available in Electron.');
}

async function openProject() {
  if (!(await confirmDiscardUnsavedChanges())) return;

  if (window.bumpforgeElectron?.openFile) {
    const result = await window.bumpforgeElectron.openFile({
      filters: _projectFilters()
    });

    if (result?.canceled || !result?.filePaths?.length) return;
    await openProjectFromPath(result.filePaths[0]);
    return;
  }
...
function resetProjectStateToStartup() {
  if (allSlotsPreviewActive) exitAllSlotsPreview();
  ...
  try {
    ...
    // Reset all material slots, not only the active one.
    textureSlots = TEXTURE_SLOT_DEFS.map((slot, i) => ({
      ...slot,
      id: `slot${i + 1}`,
      name: `Slot ${i + 1}`,
      activeMapEntry: null,
    ...
    // Reset settings without using Reset Settings button logic.
    const snapshot = { ...DEFAULT_SETTINGS_SNAPSHOT };
    Object.assign(settings, snapshot);
    updateSettingsUIFromSettings();

    // Return to startup geometry.
    loadDefaultCube();
    ...
    try {
      ...
      sessionStorage.removeItem('diorama-texture-slots');
    } catch { /* ignore */ }
  } finally {
    _autoSavePaused = false;
    _undoApplyDepth--;
...
async function newProject() {
  ...
  resetProjectStateToStartup();
  markProjectClean(null);
}

projectNewBtn?.addEventListener('click', () => {
  newProject().catch(err => alert(`New project failed: ${err.message}`));
});
 ...
 * `currentGeometry`).  We keep the mask separate from settings.json so older
 * imports that do not have it still work unchanged.
 */
function _collectCurrentMask() {
  if (!currentGeometry) return null;
  return {
    selectionMode,
    excluded: Array.from(excludedFaces || [])
...
importProjectInput?.addEventListener('change', async (e) => {
  ...
  try { await importProject(file, { filePath: null }); }
...
async function importProject(file, options = {}) {
  ...
  try {
    ...
    _autoSaveSettings();
    markProjectClean(options.filePath || null);
  } finally {
    isRestoringProject = false;
    _undoApplyDepth--;
